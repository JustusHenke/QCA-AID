...
async def main(cli_args: dict = None) -> None:
    ...
    if config_loader.get_category_loader():
        ...
        if all_valid:
            ...
            print(
                "Verwenden Sie den QCA-AID-Config-Builder.py um die Konfiguration zu korrigieren."
            )
    ...
    # Custom API Key Env aus base_config falls vorhanden
    custom_key_env = base_config.get("api_key_env", None)
    llm_provider = LLMProviderFactory.create_provider(
        PROVIDER_NAME, api_key_env=custom_key_env
    )
    ...
    harmonization_performed = analyzer.perform_selective_harmonization(
        analysis_configs, CLEAN_KEYWORDS, SIMILARITY_THRESHOLD
    )
    ...
    for config_idx, analysis_config in enumerate(analysis_configs, 1):
        ...
        if filtered_df.empty:
            print(
                f"⚠️  WARNUNG: Keine Daten nach Filterung für Analyse '{analysis_name}'"
            )
        ...
        if harmonization_performed and analyzer.needs_keyword_harmonization(
        ...
        if analysis_type == "netzwerk":
            ...
            analyzer.create_network_graph(
                filtered_df, f"Code-Network_{output_prefix}", params
            )

        elif analysis_type == "heatmap":
            # Heatmap erstellen
            print("\nErstelle Heatmap...")
            analyzer.create_heatmap(filtered_df, f"Heatmap_{output_prefix}", params)

        elif analysis_type == "sunburst":
            # Sunburst-Visualisierung erstellen
            print("\nErstelle Sunburst-Visualisierung...")
            analyzer.create_sunburst(filtered_df, f"Sunburst_{output_prefix}", params)

        elif analysis_type == "treemap":
            # Treemap-Visualisierung erstellen
            print("\nErstelle Treemap-Visualisierung...")
            analyzer.create_treemap(filtered_df, f"Treemap_{output_prefix}", params)
            ...
            prompt_template = params.get(
                "prompt_template", default_prompts.get("paraphrase", None)
            )
            ...
            # Verwende benutzerdefinierten Prompt, falls vorhanden
            prompt_template = params.get(
                "prompt_template", default_prompts.get("reasoning", None)
            )
            ...
            if not prompt_template:
                print(
                    "Warnung: Kein Prompt-Template für benutzerdefinierte Zusammenfassung angegeben. Überspringe."
                )
                continue
            ...
            await analyzer.create_sentiment_analysis(
                filtered_df, f"Sentiment_{output_prefix}", params
            )
        else:
