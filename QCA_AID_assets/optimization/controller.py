...
from QCA_AID_assets.optimization.unified_relevance_analyzer import (
...
class OptimizationController:
    ...
    def __init__(
        ...
        output_dir: str = None,
        cache_dir: str = None,
        relevance_temperature: float = 0.3,
        inductive_temperature: float = 0.2,
        ...
        reliability_db_path = (
            os.path.join(self.output_dir, "all_codings.json")
            if self.output_dir
            else "output/all_codings.json"
        )
    ...
    def _get_subcategories_for_category(
        self,
        target_category: str,
        original_subcategories: list = None,
        category_definitions: Dict = None,
    ) -> list:
        """
        ...
        if category_definitions and target_category in category_definitions:
            ...
            if hasattr(cat_def, "subcategories") and cat_def.subcategories:
                ...
                print(
                    f"      📋 Subkategorien für '{target_category}' aus aktuellen Kategorien: {target_subcategories}"
                )
        ...
        if not target_subcategories:
            ...
            if target_category in DEDUKTIVE_KATEGORIEN:
                ...
                subcats_dict = DEDUKTIVE_KATEGORIEN[target_category].get(
                    "subcategories", {}
                )
                target_subcategories = list(subcats_dict.keys())
                print(
                    f"      📋 Subkategorien für '{target_category}' aus statischen Kategorien (Fallback): {target_subcategories}"
                )
        ...
        if not target_subcategories:
            ...
            if original_subcategories:
                for subcat in original_subcategories:
                    if (
                        isinstance(subcat, dict)
                        and subcat.get("category") == target_category
                    ):
            ...
            if not target_subcategories:
                print(
                    f"      ⚠️ Keine spezifischen Subkategorien für '{target_category}' gefunden - leer gelassen für manuelle Nachbearbeitung"
                )

        return target_subcategories

    async def _code_segment_with_focus(
        self,
        segment_text: str,
        segment_id: str,
        focus_category: str,
        ...
        try:
            ...
            focused_segment = {"segment_id": segment_id, "text": segment_text}
            ...
            if analysis_mode == "deductive":
                ...
                focused_category_definitions = {
                    focus_category: category_definitions.get(
                ...
                if analysis_mode == "inductive":
                    ...
                    batch_results = await self._batch_analyze_abductive_direct(
                        ...
                        category_preselections={
                            segment_id: {"preferred_categories": [focus_category]}
                        },
                    ...
                    if "result" in batch_result:
                        ...
                        class MockResult:
                            def __init__(self, data):
                                self.primary_category = data.get(
                ...
                print(
                    f"      ⚠️ Grounded mode fokussierte Kodierung noch nicht implementiert - verwende Fallback"
                )
                return None

            if not result:
                print(
                    f"      ❌ Keine Antwort für fokussierte Kodierung: {focus_category} (Mode: {analysis_mode})"
                )
                return None
            ...
            formatted_result = {
                ...
                "result": {
                    ...
                    else focus_context["relevance_score"],
                    "all_categories": [
                    ...
                    "subcategory_validation": getattr(
                        result, "subcategory_validation", {}
                    ),
                },
                "analysis_mode": analysis_mode,
            ...
            print(
                f"      ❌ Fokussierte Kodierung Fehler für {focus_category} (Mode: {analysis_mode}): {analyzer_error}"
            )
            return None

    async def _process_focused_batch(
        self,
        focused_segments: List[Dict],
        category_definitions: Dict,
        research_question: str,
        coding_rules: List[str],
        temperature: float,
        ...
        for focused_segment in focused_segments:
            ...
            try:
                print(
                    f"      🎯 Fokussierte Kodierung: {focus_category} für {original_task['segment_id']}"
                )
                ...
                if focused_result:
                    ...
                    print(
                        f"      ✅ Fokussierte Kodierung erfolgreich: {focused_result['result']['primary_category']}"
                    )
                else:
                    print(
                        f"      ❌ Fokussierte Kodierung fehlgeschlagen - verwende Fallback"
                    )
                ...
                print(
                    f"      ❌ Fehler bei fokussierter Kodierung für {focus_category}: {e}"
                )
                # Fallback: Erstelle Basis-Ergebnis
                fallback_result = self._create_focused_fallback_result(
                    original_task,
    ...
    def _create_focused_fallback_result(
        self,
        original_task: Dict,
        focus_category: str,
        focus_context: Dict,
        ...
        """
        multiple_segment_id = (
            f"{original_task['segment_id']}-{original_task['instance_number']}"
        )
        ...
        print(
            f"      📋 Fallback Subkategorien für '{focus_category}': {target_subcategories}"
        )
        ...
        return {
            ...
            "result": {
                ...
                "preselection_reasoning": original_task["seg_prefs"].get(
                    "reasoning", ""
                ),
                # Mehrfachkodierungs-Metadaten
    ...
    def _create_fallback_multiple_coding_result(
        self,
        original_result,
        target_category: str,
        cat_info: Dict,
        ...
        target_subcategories = self._get_subcategories_for_category(
            target_category,
            original_result.subcategories
            if hasattr(original_result, "subcategories")
            else None,
        ...
        return {
            ...
            "result": {
                ...
                if hasattr(original_result, "keywords")
                else "",
                "paraphrase": original_result.paraphrase
                if hasattr(original_result, "paraphrase")
                else "",
    ...
    def _serialize_category_definitions(
        self, category_definitions: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        for name, definition in category_definitions.items():
            if hasattr(definition, "definition"):
                    ...
                    for subcat_name, subcat_obj in definition.subcategories.items():
                        if hasattr(subcat_obj, "definition"):
                            ...
                            serialized_subcategories[subcat_name] = {
                                ...
                                "examples": subcat_obj.examples
                                if hasattr(subcat_obj, "examples")
                                else [],
                                "rules": subcat_obj.rules
                                if hasattr(subcat_obj, "rules")
                                else [],
                            }
                        else:
                ...
                serializable_definitions[name] = {
                    ...
                    if hasattr(definition, "examples")
                    else [],
    ...
    async def _store_relevance_results_for_export(
        self, all_relevance_results: List[Dict], all_segments: List[Dict]
    ) -> None:
        """
        ...
        if (
            hasattr(self, "analysis_manager")
            and self.analysis_manager
            and hasattr(self.analysis_manager, "relevance_checker")
        ):
            relevance_checker = self.analysis_manager.relevance_checker

        if not relevance_checker:
            print(
                "   ⚠️ WARNING: No RelevanceChecker available to store results for export"
            )
            return

        print(
            f"   📝 Storing {len(all_relevance_results)} relevance results (relevant + non-relevant) in RelevanceChecker for export..."
        )

        # Create a map of ALL segments with their LLM results
        all_results_map = {
            result.get("segment_id", ""): result for result in all_relevance_results
        }
        ...
        for segment in all_segments:
            ...
            if segment_id in all_results_map:
                ...
                aspects_found = rel_result.get(
                    "aspects_found", rel_result.get("core_topics_found", [])
                )
                key_aspects = rel_result.get("key_aspects", [])
                reasoning = rel_result.get(
                    "relevance_reasoning",
                    rel_result.get(
                        "reasoning",
                        rel_result.get(
                            "justification",
                            "Keine spezifische Begründung vom LLM erhalten",
                        ),
                    ),
                )
                ...
                if not aspects_found and not key_aspects and reasoning:
                    ...
                    if extracted_aspects:
                        ...
                        aspects_found = (
                            extracted_aspects  # All aspects mentioned in reasoning
                        )
                        ...
                        if not key_aspects:
                            key_aspects = (
                                extracted_aspects[:2]
                                if len(extracted_aspects) > 1
                                else extracted_aspects
                            )
                        else:
                ...
                actual_classification_confidence = rel_result.get(
                ...
                if (
                    actual_classification_confidence is None
                    or actual_classification_confidence == 0.0
                ):
                    ...
                    actual_classification_confidence = min(
                        0.95, (reasoning_quality * 0.4) + (relevance_strength * 0.6)
                    )
                    classification_confidence_source = "calculated"
                    print(
                        f"   🧮 DEBUG: Calculated classification_confidence for {segment_id}: {actual_classification_confidence:.2f} (reasoning_quality: {reasoning_quality:.2f}, relevance_strength: {relevance_strength:.2f})"
                    )
                ...
                relevance_checker.relevance_details[segment_id] = {
                    "confidence": rel_result.get(
                        "research_relevance", rel_result.get("confidence", 0.8)
                    ),
                    "relevance_strength": rel_result.get(
                        "research_relevance", rel_result.get("relevance_strength", 0.8)
                    ),
                ...
                print(
                    f"   ⚠️ WARNING: No LLM result found for segment {segment_id} - using fallback"
                )
                ...
                random.seed(
                    hash(segment_id)
                )  # Deterministic but varied based on segment_id
                non_relevant_confidence = 0.75 + (
                    random.random() * 0.2
                )  # Range: 0.75-0.95
        ...
        print(
            f"   ✅ Stored relevance details for {len(relevance_checker.relevance_details)} segments"
        )
    ...
    def _assess_reasoning_quality(self, reasoning: str) -> float:
        ...
        positive_indicators = [
            ...
            ("detailliert", 0.1),
            ("beispiel", 0.1),
            ("bezieht sich auf", 0.1),
            ("behandelt", 0.1),
            ("thematisiert", 0.1),
            ("informationen zu", 0.1),
            ("aspekte", 0.1),
            ("relevant für", 0.1),
            ("beiträgt zu", 0.1),
            ("direkt", 0.1),
    ...
    def _extract_aspects_from_reasoning(self, reasoning: str) -> List[str]:
        ...
        for pattern in aspect_patterns:
            ...
            for match in matches:
                ...
                if (
                    len(aspect) > 5 and aspect not in aspects
    ...
    async def analyze_segments(
        ...
        **kwargs,
        ...
        if coder_settings:
            ...
            print(
                f"   🔧 Cache-Manager konfiguriert: {len(coder_settings)} Kodierer, Modus: {analysis_mode.value}"
            )
            ...
            print(
                f"   👤 Coder-Specific Operations: {strategy_info['coder_specific_operations']}"
            )
        else:
            # Default single coder configuration
            default_coder_settings = [
                {"coder_id": "auto_1", "temperature": temperature or 0.3}
            ]
            self.dynamic_cache_manager.configure_coders(default_coder_settings)
            print(
                f"   🔧 Cache-Manager konfiguriert: Standard Single-Coder, Modus: {analysis_mode.value}"
            )

        # Override batch_size if provided by user
        if batch_size is not None:
            print(
                f"   ⚙️ User-Config Batch Size: {batch_size} (überschreibt Standard-Werte)"
            )
        ...
        if use_context:
            print(
                f"   📝 Kontext-Paraphrasen aktiviert: max. {context_paraphrase_count} pro Dokument"
            )
            if document_paraphrases:
                total_paraphrases = sum(
                    len(paraphrases) for paraphrases in document_paraphrases.values()
                )
                print(
                    f"   📝 Verfügbare Paraphrasen: {total_paraphrases} aus {len(document_paraphrases)} Dokumenten"
                )
            else:
        ...
        try:
            ...
            if analysis_mode == AnalysisMode.DEDUCTIVE:
                ...
                updated_categories = getattr(
                    self, "_last_developed_categories", current_categories
                )
                ...
                updated_categories = getattr(
                    self, "_last_extended_categories", current_categories
                )
                return results, updated_categories
            ...
            if hasattr(self, "_debug_expected_calls"):
                ...
                if total_requests != self._debug_expected_calls:
                    print(
                        f"   ⚠️ DEBUG API Call Diskrepanz: Erwartet {self._debug_expected_calls}, Tatsächlich {total_requests}"
                    )
    ...
    async def _analyze_deductive(
        ...
        **kwargs,
        ...
        if coder_settings:
            print(
                f"[DEBUG _analyze_deductive] Anzahl coder_settings: {len(coder_settings)}"
            )
            for cs in coder_settings:
                print(
                    f"  - {cs.get('coder_id', '?')}: temp={cs.get('temperature', '?')}"
                )
        ...
        preference_calls = (
            estimated_batches  # Category preferences are batched but shared
        )
        ...
        if coder_settings and len(coder_settings) > 1:
            ...
            print(
                f"   🔄 Multi-Coder Mode: Intelligente Cache-Verwaltung für {len(coder_settings)} Kodierer"
            )
            ...
            print(
                f"      └─ Shared calls: {expected_calls_base} ({relevance_calls} relevance + {preference_calls} preferences batches)"
            )
            print(
                f"      └─ Coding calls: {estimated_coding_calls} ({len(coder_settings)} coders × {estimated_batches} batches)"
            )
            print(f"      └─ Segments: {len(segments)}, Batch size: {batch_size}")
            print(
                f"      └─ Formula: {estimated_batches} × (2 shared + {len(coder_settings)} coders) = {estimated_batches} × {2 + len(coder_settings)} = {self._debug_expected_calls}"
            )
            ...
            print(
                f"   📊 Schritt 1: Erweiterte Relevanzprüfung mit Kategorie-Präferenzen (SHARED)..."
            )
            print(f"   📋 Forschungsfrage: {research_question}")

            # Schritt 1a: Einfache Relevanzprüfung (SHARED) - EINMAL für beide Zwecke
            print(
                f"   🔍 API Call 1: Einfache Relevanzprüfung für {len(segments)} Segmente..."
            )
            ...
            await self._store_relevance_results_for_export(
                all_relevance_results, segments
            )
            ...
            relevance_results = [
                ...
                and result.get("research_relevance", 0.0) >= relevance_threshold
            ]
            ...
            if not relevant_segments:
                print(
                    f"   ✅ 0 von {len(segments)} Segmenten relevant - KEINE weiteren API Calls nötig"
                )
                return []

            print(
                f"   ✅ {len(relevant_segments)} von {len(segments)} Segmenten relevant"
            )

            # Schritt 1b: Kategoriepräferenzen für relevante Segmente (SHARED)
            print(
                f"   🔍 API Call 2: Kategoriepräferenzen für {len(relevant_segments)} relevante Segmente..."
            )
            category_preference_results = (
                await self.unified_analyzer.analyze_category_preferences(
            ...
            for pref_result in category_preference_results:
                ...
                if seg_id:
                    ...
                    if preferred_cats:
                        print(
                            f"      └─ Präferierte Kategorien: {', '.join(preferred_cats)}"
                        )
                        ...
                        print(
                            f"      └─ Keine starken Kategoriepräferenzen (alle Scores < 0.6)"
                        )

            print(
                f"   ✅ Kategoriepräferenzen für {len(category_preselections)}/{len(relevant_segments)} Segmente ermittelt"
            )
            ...
            if category_preselections:
                ...
                print(
                    f"   📊 Kategorie-Präferenzen Zusammenfassung: {dict(pref_counts)}"
                )
            else:
                print(
                    f"   ⚠️ Keine Kategoriepräferenzen ermittelt - alle Segmente haben schwache Kategorie-Scores"
                )
            ...
            print(
                f"      └─ Estimation accuracy: {optimization_calls}/{self._debug_expected_calls} = {(optimization_calls / self._debug_expected_calls * 100):.1f}%"
                if self._debug_expected_calls > 0
                else ""
            )
            ...
            # Single coder mode
            coder_id = (
                coder_settings[0].get("coder_id", "auto_1")
                if coder_settings
                else "auto_1"
            )
            ...
            print(
                f"   🔍 DEBUG Geschätzte API Calls (Single-Coder): {self._debug_expected_calls}"
            )
            print(
                f"      └─ Shared calls: {expected_calls_base} ({relevance_calls} relevance + {preference_calls} preferences batches)"
            )
            print(
                f"      └─ Coding calls: {estimated_coding_calls} (1 coder × {estimated_batches} batches)"
            )
            print(f"      └─ Segments: {len(segments)}, Batch size: {batch_size}")
            print(
                f"      └─ Formula: {estimated_batches} × (2 shared + 1 coder) = {estimated_batches} × 3 = {self._debug_expected_calls}"
            )
    ...
    def _get_progressive_context_paraphrases(
        self,
        segment_id: str,
        ...
        if context_paraphrases:
            print(
                f"      📝 Kontext für {segment_id}: {len(context_paraphrases)} Paraphrasen aus Chunks 0-{current_chunk_num - 1}"
            )
            for i, paraphrase in enumerate(context_paraphrases):
                # Kürze Paraphrase für Logging
                short_paraphrase = (
                    paraphrase[:100] + "..." if len(paraphrase) > 100 else paraphrase
                )
                print(f"         {i + 1}. {short_paraphrase}")

        return context_paraphrases

    def _format_single_coding_result(
        self, r, coder_id: str, preferred_cats: List[str], seg_prefs: Dict
    ) -> Dict:
        """
    ...
    def _format_single_coding_result_inductive(self, r, coder_id: str) -> Dict:
        ...
        # Enhanced logging
        seg_id_short = (
            r.segment_id[:30] + "..." if len(r.segment_id) > 30 else r.segment_id
        )
    ...
    def _format_single_coding_result_abductive(
        self, r, coder_id: str, preferred_cats: List[str], seg_prefs: Dict
    ) -> Dict:
        """
    ...
    async def _analyze_deductive_coding_only(
        self,
        ...
        print(
            f"   📊 Schritt 3: Kodierung von {len(relevant_segments)} relevanten Segmenten..."
        )
        ...
        for i in range(0, len(relevant_segments), batch_size):
            ...
            if config["enable_batching"] and len(batch) > 1:
                ...
                use_temperature = (
                    ...
                    else self.unified_analyzer.temperature
                )
                for segment in batch:
                    ...
                    if preferred_cats:
                        ...
                        print(
                            f"   🎯 Segment {segment['segment_id']}: Fokus auf {len(effective_categories)} präferierte Kategorien: {list(effective_categories.keys())}"
                        )
                        print(
                            f"      └─ Ursprünglich {len(category_definitions)} Kategorien verfügbar: {list(category_definitions.keys())}"
                        )
                    else:
                        print(
                            f"   📝 Segment {segment['segment_id']}: Keine Kategoriepräferenzen - verwende alle {len(effective_categories)} Kategorien"
                        )
                    ...
                    if result:
                        print(
                            f"      🔍 DEBUG: LLM returned category '{result.primary_category}' for segment with preferred categories {preferred_cats}"
                        )

                        # CRITICAL FIX: Validate that returned category is in the filtered set
                        if (
                            preferred_cats
                            and result.primary_category not in preferred_cats
                        ):
                            print(
                                f"      ❌ WARNING: LLM chose '{result.primary_category}' but preferred categories were {preferred_cats}"
                            )
                            print(
                                f"      🔧 FIXING: Forcing category to first preferred category: {preferred_cats[0]}"
                            )
                            ...
                            original_justification = getattr(
                        ...
                        formatted_result = {
                            ...
                            "result": {
                                "primary_category": result.primary_category,
                                "confidence": result.confidence,
                                "all_categories": getattr(
                                    result, "all_categories", [result.primary_category]
                                ),
                                ...
                                "category_preselection_used": bool(preferred_cats),
                                "preferred_categories": preferred_cats,
                                "preselection_reasoning": seg_prefs.get(
                                    "reasoning", ""
                                ),
                            },
                            "analysis_mode": analysis_mode,
    ...
    async def _analyze_deductive_single(
        self,
        segments: List[Dict[str, Any]],
        category_definitions: Dict[str, str],
        research_question: str,
        ...
        print(
            f"   📊 Schritt 1: Erweiterte Relevanzprüfung mit Kategorie-Präferenzen..."
        )
        print(f"   📋 Forschungsfrage: {research_question}")
        ...
        for pref_result in category_preference_results:
            ...
            if seg_id:
                ...
                if top_categories:
                    print(
                        f"      └─ Präferierte Kategorien: {', '.join(top_categories)}"
                    )
                    # Show scores for preferred categories
        ...
        if category_preselections:
            ...
            print(
                f"   📊 Kategorie-Präferenzen Zusammenfassung: {dict(preselection_stats)}"
            )
        else:
            print(
                f"   ⚠️ Keine Kategoriepräferenzen ermittelt - alle Segmente haben schwache Kategorie-Scores"
            ...
            if config["enable_batching"] and len(batch) > 1:
                ...
                for segment in batch:
                    ...
                    if preferred_cats:
                        ...
                        print(
                            f"   🎯 Segment {segment['segment_id']}: Fokus auf {len(effective_categories)} präferierte Kategorien"
                        )
    ...
    async def _batch_analyze_deductive(
        self,
        ...
        for segment in batch:
            ...
            serializable_category_definitions = self._serialize_category_definitions(
                category_definitions
            )
            ...
            if cached_val:
                ...
                if (
                    self.dynamic_cache_manager.should_cache_shared("coding")
                    or cached_coder_id == coder_id
                ):
        ...
        print(
            f"      📊 Cache Performance: {cache_hits} Hits, {cache_misses} Misses ({hit_rate:.1f}% Hit Rate)"
        )
        ...
        if uncached_segments:
            print(
                f"      🔍 API Call 3+: Kodierung von {len(uncached_segments)} Segmenten (Cache Miss)"
            )
            ...
            if category_preselections:
                ...
                if segments_with_prefs > 0 and all_preferred_cats:
                    ...
                    print(
                        f"   🎯 Batch: Verwende {len(effective_categories)} präferierte Kategorien mit segment-spezifischen Einschränkungen"
                    )
            ...
            if use_context:
                print(
                    f"      🔍 DEBUG Kontext: use_context={use_context}, document_paraphrases={'verfügbar' if document_paraphrases else 'None/leer'}"
                )
                if document_paraphrases:
                    print(
                        f"         └─ Dokumente mit Paraphrasen: {list(document_paraphrases.keys())}"
                    )
            ...
            if use_context and document_paraphrases and uncached_segments:
                ...
                if context_paraphrases:
                    ...
                    chunk_num = self._extract_chunk_number_from_segment_id(
                        first_segment_id
                    )
                    print(
                        f"      📝 ✅ VERWENDE {len(context_paraphrases)} progressive Kontext-Paraphrasen für Dokument '{doc_name}' (Chunk {chunk_num})"
                    )
                    print(
                        f"         └─ Kontext-Paraphrasen werden an LLM gesendet für bessere Kodierung"
                    )
                else:
                    first_segment_id = uncached_segments[0]["segment_id"]
                    doc_name = self._extract_doc_name_from_segment_id(first_segment_id)
                    ...
                    print(
                        f"      📝 ❌ KEINE Kontext-Paraphrasen für Dokument '{doc_name}' (Chunk {chunk_num}) - erstes Segment oder keine verfügbar"
                    )
                    doc_name = self._extract_doc_name_from_segment_id(first_segment_id)
                    chunk_num = self._extract_chunk_number_from_segment_id(
                        first_segment_id
                    ...
                    print(
                        f"      📝 Keine Kontext-Paraphrasen für Dokument '{doc_name}' Chunk {chunk_num} (erste Chunks oder nicht verfügbar)"
                    )
            ...
            segment_text_map = {
                seg["segment_id"]: seg["text"] for seg in uncached_segments
            }
            ...
            for r in batch_results:
                ...
                seg_prefs = (
                    category_preselections.get(r.segment_id, {})
                    if category_preselections
                    else {}
                )
                preferred_cats = seg_prefs.get("preferred_categories", [])
                ...
                if preferred_cats and r.primary_category not in preferred_cats:
                    print(
                        f"      ❌ BATCH WARNING: LLM chose '{r.primary_category}' but preferred categories were {preferred_cats}"
                    )
                    print(
                        f"      🔧 BATCH FIXING: Forcing category to first preferred category: {preferred_cats[0]}"
                    )
                    ...
                    for cat_name, score in category_preferences.items():
                        if score >= self.unified_analyzer.multiple_coding_threshold:
                            relevant_categories.append(
                                {"category": cat_name, "relevance_score": score}
                            )

                # Mehrfachkodierung wenn 2+ Kategorien über Schwellenwert
                if len(relevant_categories) > 1:
                    print(
                        f"   🔁 Mehrfachkodierung für Segment {r.segment_id}: {len(relevant_categories)} Kategorien über Schwellenwert {self.unified_analyzer.multiple_coding_threshold}"
                    )

                    # Erste Kodierung: Verwende erste (höchste) Kategorie
                    first_category = relevant_categories[0]
                    first_result = self._format_single_coding_result(
                        r, coder_id, preferred_cats, seg_prefs
                    )
                    # Überschreibe mit erster Kategorie
                    first_result["result"]["primary_category"] = first_category[
                        "category"
                    ]
                    first_result["result"]["justification"] = (
                        f"[Mehrfachkodierung 1/{len(relevant_categories)}, Score: {first_category['relevance_score']:.2f}] {r.justification if hasattr(r, 'justification') else ''}"
                    )
                    first_result["result"]["is_multiple_coding"] = True
                    first_result["result"]["multiple_coding_instance"] = 1
                    first_result["result"]["total_coding_instances"] = len(
                        relevant_categories
                    )
            ...
            if segments_needing_multiple_coding:
                print(
                    f"   🎯 Fokussierter Batch: {len(segments_needing_multiple_coding)} zusätzliche Kodierungen"
                )
            ...
            for formatted_res in new_results:
                ...
                seg_id_short = (
                    segment_id[:30] + "..." if len(segment_id) > 30 else segment_id
                )
                print(
                    f"   📝 Segment: {seg_id_short} → Kategorie: {result_data['primary_category']}"
                )
                if result_data.get("subcategories"):
                    print(f"      └─ Subkategorien: {result_data['subcategories']}")
                if result_data.get("preferred_categories"):
                    print(
                        f"      └─ Präferierte Kategorien: {result_data['preferred_categories']}"
                    )
                print(
                    f"      └─ Kodierer: {coder_id} | Konfidenz: {result_data['confidence']:.2f}"
                )
                if result_data.get("justification"):
                    justification_short = (
                        result_data["justification"][:80] + "..."
                        if len(result_data["justification"]) > 80
                        else result_data["justification"]
                    )
                    print(f"      └─ Begründung: {justification_short}")
                if result_data.get("is_multiple_coding"):
                    print(
                        f"      └─ Mehrfachkodierung: {result_data['multiple_coding_instance']}/{result_data['total_coding_instances']}"
                    )
                ...
                # Add to cache
                serializable_category_definitions = (
                    self._serialize_category_definitions(category_definitions)
                )
                ...
                self.cache.set(
                    ...
                    segment_text=next(
                        (
                            s["text"]
                            for s in uncached_segments
                            if s["segment_id"] == cache_segment_id
                        ),
                        "",
                    ),
                    value=formatted_res,
                ...
                reliability_result = ExtendedCodingResult(
                    ...
                    metadata={
                        ...
                        "is_multiple_coding": result_data.get(
                            "is_multiple_coding", False
                        ),
                        "multiple_coding_instance": result_data.get(
                            "multiple_coding_instance", 1
                        ),
                        "total_coding_instances": result_data.get(
                            "total_coding_instances", 1
                        ),
                        "original_segment_id": result_data.get(
                            "original_segment_id", segment_id
                        ),
                    },
                )
    ...
    async def _store_results_for_reliability(
        self,
        ...
        for result in results:
            try:
                ...
                if "result" in result:
                    ...
                    primary_category = result_data.get(
                        "primary_category", "Nicht kodiert"
                    )
                ...
                actual_temperature = (
                    temperature
                    if temperature is not None
                    else getattr(self, f"{analysis_mode}_temperature", 0.3)
                )
    ...
    async def _analyze_inductive(
        ...
        **kwargs,
        ...
        from QCA_AID_assets.analysis.saturation_controller import (
        ...
        print(
            f"   ✅ {len(relevant_segments)} von {len(segments)} Segmenten sind relevant"
        )
        ...
        print(
            f"   🔍 Schritt 2: Kategorienentwicklung aus {len(relevant_segments)} relevanten Segmenten..."
        )

        # Berechne Batch-Parameter für echtes Batch-Tracking
        category_batch_size = min(
            batch_size * 2, 10
        )  # Größere Batches für Kategorienentwicklung
        total_category_batches = (
            len(relevant_segments) + category_batch_size - 1
        ) // category_batch_size

        print(
            f"   📦 Kategorienentwicklung in {total_category_batches} Batches (Batch-Größe: {category_batch_size})"
        )
        ...
        for batch_idx in range(total_category_batches):
            ...
            print(
                f"   🔍 API Call 2.{batch_number}: Induktive Kategorienentwicklung für Batch {batch_number}/{total_category_batches} ({len(batch_segments)} Segmente)"
            )
            ...
            batch_categories = self._extract_categories_from_inductive_results(
                category_results, developed_categories
            )
            ...
            if new_categories_count > 0:
                print(
                    f"   ✅ Batch {batch_number}: {new_categories_count} neue Kategorien entwickelt"
                )
            ...
            print(
                f"      - Theoretische Sättigung: {saturation_assessment['theoretical_saturation']:.2f}"
            )
            print(
                f"      - Materialabdeckung: {saturation_assessment['material_coverage']:.1%}"
            )
            print(
                f"      - Stabilität: {saturation_assessment['stability_batches']} Batches"
            )
            print(
                f"      - Kategorienqualität: {saturation_assessment['category_quality']:.2f}"
            )
            print(
                f"      - Sättigung erreicht: {'✅ JA' if saturation_assessment['is_saturated'] else '❌ NEIN'}"
            )
            ...
            if saturation_assessment["is_saturated"] and material_coverage >= 0.7:
                print(
                    f"   🎯 SÄTTIGUNG ERREICHT nach Batch {batch_number}/{total_category_batches}"
                )
                print(f"      Grund: {saturation_assessment['saturation_reason']}")
                break
            elif batch_number < total_category_batches:
                print(
                    f"   ➡️ Setze Kategorienentwicklung fort (Batch {batch_number + 1}/{total_category_batches})"
                )

        # Finale Sättigungsauswertung
        final_material_coverage = (
            len(relevant_segments) / len(segments) if segments else 0.0
        )
        ...
        print(
            f"      - Theoretische Sättigung: {final_saturation['theoretical_saturation']:.2f}"
        )
        print(f"      - Materialabdeckung: {final_saturation['material_coverage']:.1%}")
        print(
            f"      - Stabilität: {final_saturation['stability_batches']} stabile Batches"
        )
        print(f"      - Kategorienqualität: {final_saturation['category_quality']:.2f}")
        print(
            f"      - Endstatus: {'🎯 GESÄTTIGT' if final_saturation['is_saturated'] else '⚠️ NICHT GESÄTTIGT'}"
        )
        ...
        if coder_settings and len(coder_settings) > 1:
            ...
            print(
                f"   🔄 Single-Coder Mode: Kodierung mit '{coder_id}' (Temperature: {coder_temp})"
            )
            ...
            await self._store_results_for_reliability(
                ...
                "inductive",
                "auto_fallback",
                temperature or self.inductive_temperature,
            )
    ...
    async def _analyze_inductive_coding_only(
        self,
        segments: List[Dict[str, Any]],
        category_definitions: Dict[str, str],
        research_question: str,
        ...
        for i in range(0, len(segments), batch_size):
            ...
            if use_context:
                print(
                    f"      🔍 DEBUG Kontext (inductive): use_context={use_context}, document_paraphrases={'verfügbar' if document_paraphrases else 'None/leer'}"
                )
                if document_paraphrases:
                    print(
                        f"         └─ Dokumente mit Paraphrasen: {list(document_paraphrases.keys())}"
            ...
            if use_context and document_paraphrases and batch:
                ...
                if context_paraphrases:
                    ...
                    print(
                        f"      📝 ✅ VERWENDE {len(context_paraphrases)} progressive Kontext-Paraphrasen für Dokument '{doc_name}' (Chunk {chunk_num}) (inductive)"
                    )
                    print(
                        f"         └─ Kontext-Paraphrasen werden an LLM gesendet für bessere Kodierung"
                    )
                    ...
                    print(
                        f"      📝 ❌ KEINE Kontext-Paraphrasen für Dokument '{doc_name}' (Chunk {chunk_num}) - erstes Segment oder keine verfügbar (inductive)"
                    )
                    chunk_num = self._extract_chunk_number_from_segment_id(
                        first_segment_id
                    )
                    print(
                        f"      📝 Keine Kontext-Paraphrasen für Dokument '{doc_name}' Chunk {chunk_num} (erste Chunks oder nicht verfügbar) (inductive)"
                    )
            ...
            for r in batch_results:
                ...
                if not available_categories:
                    ...
                    print(
                        f"   ℹ️ Inductive Mode: Keine Kategorien verfügbar für Segment {r.segment_id} -> keine Mehrfachkodierung"
                    )
                ...
                if hasattr(r, "relevance_scores") and r.relevance_scores:
                    ...
                    for cat_name, score in r.relevance_scores.items():
                        if (
                            cat_name in available_categories
                            and score >= self.unified_analyzer.multiple_coding_threshold
                        ):
                            relevant_categories.append(
                                {"category": cat_name, "relevance_score": score}
                            )
                ...
                if len(relevant_categories) > 1:
                    print(
                        f"   🔁 Mehrfachkodierung für Segment {r.segment_id}: {len(relevant_categories)} verfügbare Kategorien über Schwellenwert {self.unified_analyzer.multiple_coding_threshold} (inductive)"
                    )

                    # Erste Kodierung: Verwende erste (höchste) Kategorie
                    first_category = relevant_categories[0]
                    first_result = self._format_single_coding_result_inductive(
                        r, coder_id
                    )
                    # Überschreibe mit erster Kategorie
                    first_result["result"]["primary_category"] = first_category[
            ...
            if segments_needing_multiple_coding:
                print(
                    f"   🎯 Fokussierter Batch (inductive): {len(segments_needing_multiple_coding)} zusätzliche Kodierungen"
                )
                focused_results = await self._process_focused_batch(
                    focused_segments=segments_needing_multiple_coding,
                    category_definitions=category_definitions,
    ...
    def _extract_categories_from_inductive_results(
        self, category_results: List[Dict[str, Any]], current_categories: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        for result in category_results:
            ...
            if "development_assessment" in result:
                ...
                print(
                    f"   📊 Sättigungsmetriken: Sättigung={saturation:.2f}, Empfehlung={recommendation}"
                )
            ...
            if "categories" in result:
                ...
                for cat_data in new_categories:
                    ...
                    if cat_name:
                        ...
                        category_definitions[cat_name] = cat_data.get(
                            "definition", f"Induktiv entwickelte Kategorie: {cat_name}"
                        )
            ...
            for cat_name in categories:
                if (
                    cat_name
                    and cat_name not in developed
                    and cat_name not in current_categories
                ):
                    ...
                    if cat_name in subcategories:
                        for subcat_data in subcategories[cat_name]:
                            if isinstance(subcat_data, dict):
                                ...
                                subcat_definition = subcat_data.get(
                                    "definition", f"Subkategorie: {subcat_name}"
                                )
                            else:
                    ...
                    developed[cat_name] = CategoryDefinition(
                        name=cat_name,
                        definition=category_definitions.get(
                            cat_name, f"Induktiv entwickelte Kategorie: {cat_name}"
                        ),
                        examples=[],
                        rules=[],
                    ...
                    print(
                        f"   ✅ Neue Kategorie entwickelt: '{cat_name}' mit {len(subcat_dict)} Subkategorien"
                    )

        # Log finale Sättigungsmetriken
        if saturation_metrics:
            avg_saturation = sum(
                m.get("theoretical_saturation", 0.0) for m in saturation_metrics
            ) / len(saturation_metrics)
            recommendations = [
                m.get("recommendation", "continue") for m in saturation_metrics
            ]
    ...
    async def _analyze_abductive(
        ...
        **kwargs,
        ...
        if coder_settings:
            print(
                f"[DEBUG _analyze_abductive] Anzahl coder_settings: {len(coder_settings)}"
            )
            for cs in coder_settings:
                print(
                    f"  - {cs.get('coder_id', '?')}: temp={cs.get('temperature', '?')}"
        ...
        relevance_calls = estimated_batches  # Relevance is batched but shared
        subcategory_calls = (
            estimated_batches  # Subkategorie-Entwicklung is batched but shared
        )
        expected_calls_base = (
            relevance_calls + subcategory_calls
        )  # Both are batched and shared
        ...
        print(
            f"   🔍 DEBUG: Einfache Relevance check returned {len(all_relevance_results)} total results"
        )
        ...
        print(
            f"   🔍 Schritt 2: Erweitere bestehende Kategorien um neue Subkategorien (aus {len(relevant_segments)} relevanten Segmenten)..."
        )
        ...
        # Berechne Batch-Parameter für echtes Batch-Tracking
        subcategory_batch_size = min(
            batch_size * 2, 8
        )  # Kleinere Batches für Subkategorien-Entwicklung
        total_subcategory_batches = (
            len(relevant_segments) + subcategory_batch_size - 1
        ) // subcategory_batch_size

        print(
            f"   📦 Subkategorien-Entwicklung in {total_subcategory_batches} Batches (Batch-Größe: {subcategory_batch_size})"
        )
        ...
        for batch_idx in range(total_subcategory_batches):
            ...
            print(
                f"   🔍 API Call 3.{batch_number}: Abduktive Subkategorien-Entwicklung für Batch {batch_number}/{total_subcategory_batches} ({len(batch_segments)} Segmente)"
            )

            abductive_temp = temperature or kwargs.get(
                "temperature", self.inductive_temperature
            )
            ...
            if new_subcategories_count > 0:
                print(
                    f"   ✅ Batch {batch_number}: {new_subcategories_count} neue Subkategorien entwickelt"
                )
                saturation_controller.reset_stability_counter()
            else:
                print(
                    f"   ⚠️ Batch {batch_number}: Keine neuen Subkategorien entwickelt"
                )
                saturation_controller.increment_stability_counter()
            ...
            if saturation_assessment["is_saturated"] and material_coverage >= 0.6:
                print(
                    f"   🎯 SUBKATEGORIEN-SÄTTIGUNG ERREICHT nach Batch {batch_number}/{total_subcategory_batches}"
                )
                print(f"      Grund: {saturation_assessment['saturation_reason']}")
                break
            elif batch_number < total_subcategory_batches:
                print(
                    f"   ➡️ Setze Subkategorien-Entwicklung fort (Batch {batch_number + 1}/{total_subcategory_batches})"
                )
        ...
        for k, v in extended_categories.items():
            if hasattr(v, "definition"):
                # CategoryDefinition object
                cat_defs[k] = {"definition": v.definition, "subcategories": {}}
                # Serialisiere auch Subkategorien
                if hasattr(v, "subcategories") and v.subcategories:
                    for subcat_name, subcat_obj in v.subcategories.items():
                        if hasattr(subcat_obj, "definition"):
                            cat_defs[k]["subcategories"][subcat_name] = (
                                subcat_obj.definition
                            )
                        else:
                ...
                # String or other type
                cat_defs[k] = {"definition": str(v), "subcategories": {}}

        print(
            f"   🔧 DEBUG: Serialisierte {len(cat_defs)} Kategorien für Cache-kompatible Kodierung"
        )

        # 2.5. Kategoriepräferenzen für relevante Segmente bestimmen (NACH Subkategorien-Entwicklung!)
        print(
            f"   📊 Schritt 2.5: Kategoriepräferenzen basierend auf erweiterten Kategorien für {len(relevant_segments)} relevante Segmente..."
        )
        print(
            f"   🔍 API Call N: Kategoriepräferenzen für {len(relevant_segments)} relevante Segmente mit erweiterten Kategorien..."
        )
        ...
        for pref_result in category_preference_results:
            ...
            if seg_id:  # Segment ist bereits als relevant bestätigt
                ...
                if top_categories:
                    print(
                        f"      └─ Präferierte Kategorien (erweitert): {', '.join(top_categories)}"
                    )
                    # Show scores for preferred categories
                    for cat in top_categories:
                        score = category_preferences.get(cat, 0.0)
        ...
        if category_preselections:
            ...
            for prefs in category_preselections.values():
                for cat in prefs["preferred_categories"]:
                    preselection_stats[cat] = preselection_stats.get(cat, 0) + 1
            print(
                f"   📊 Kategorie-Präferenzen Zusammenfassung (erweiterte Kategorien): {dict(preselection_stats)}"
            )
        else:
            print(
                f"   ⚠️ Keine Kategoriepräferenzen ermittelt - alle Segmente haben schwache Kategorie-Scores"
        ...
        if coder_settings and len(coder_settings) > 1:
            ...
            # Store results for reliability analysis
            await self._store_results_for_reliability(
                coder_results, "abductive", coder_id, coder_temp
            )
            ...
            # Store results for reliability analysis
            await self._store_results_for_reliability(
                fallback_results, "abductive", "auto_fallback", temperature
            )
        ...
        if coder_settings and len(coder_settings) > 1:
            ...
            self._debug_expected_calls = expected_calls_base + estimated_coding_calls
            print(
                f"   🔍 DEBUG Geschätzte API Calls (Multi-Coder): {self._debug_expected_calls}"
            )
            print(
                f"      └─ Shared calls: {expected_calls_base} ({relevance_calls} relevance + {subcategory_calls} subcategory batches)"
            )
            print(
                f"      └─ Coding calls: {estimated_coding_calls} ({len(coder_settings)} coders × {estimated_batches} batches)"
            )
    ...
    def _extend_categories_with_subcategories(
        self,
            ...
            for result in subcategory_results:
                ...
                if extended_categories_data:
                    ...
                    for (
                        hauptkategorie_name,
                        kategorie_data,
                    ) in extended_categories_data.items():
                        new_subcategories = kategorie_data.get("new_subcategories", [])

                        if (
                            new_subcategories
                            and hauptkategorie_name in category_definitions
                        ):
                            ...
                            if hauptkategorie_name not in extended:
                                ...
                                extended[hauptkategorie_name] = CategoryDefinition(
                                    ...
                                    definition=category_definitions[
                                        hauptkategorie_name
                                    ],
                                    examples=[],
                                    rules=[],
                            ...
                            if hasattr(cat, "subcategories"):
                                for subcat_data in new_subcategories:
                                    ...
                                    subcat_definition = subcat_data.get(
                                        "definition",
                                        f"Abduktiv entwickelte Subkategorie: {subcat_name}",
                                    )

                                    if (
                                        subcat_name
                                        and subcat_name not in cat.subcategories
                                    ):
                                        current_date = datetime.now().strftime(
                                            "%Y-%m-%d"
                                        )
                                        cat.subcategories[subcat_name] = (
                                            CategoryDefinition(
                                                name=subcat_name,
                                                definition=subcat_definition,
                                                examples=list(
                                                    subcat_data.get("evidence", [])
                                                ),
                                                rules=[],
                                                subcategories={},
                                                added_date=current_date,
                                        ...
                                        print(
                                            f"      ✅ Neue Subkategorie '{subcat_name}' zu '{hauptkategorie_name}' hinzugefügt"
                                        )
                else:
    ...
    async def _batch_analyze_abductive_direct(
        self,
        batch: List[Dict[str, Any]],
        category_definitions: Dict[str, str],
        research_question: str,
        ...
        if use_context:
            print(
                f"      🔍 DEBUG Kontext (abductive): use_context={use_context}, document_paraphrases={'verfügbar' if document_paraphrases else 'None/leer'}"
            )
            if document_paraphrases:
                print(
                    f"         └─ Dokumente mit Paraphrasen: {list(document_paraphrases.keys())}"
        ...
        if use_context and document_paraphrases and batch:
            ...
            if context_paraphrases:
                ...
                print(
                    f"      📝 ✅ VERWENDE {len(context_paraphrases)} progressive Kontext-Paraphrasen für Dokument '{doc_name}' (Chunk {chunk_num}) (abductive)"
                )
                print(
                    f"         └─ Kontext-Paraphrasen werden an LLM gesendet für bessere Kodierung"
                )
                ...
                chunk_num = self._extract_chunk_number_from_segment_id(first_segment_id)
                print(
                    f"      📝 ❌ KEINE Kontext-Paraphrasen für Dokument '{doc_name}' (Chunk {chunk_num}) - erstes Segment oder keine verfügbar (abductive)"
                )
                chunk_num = self._extract_chunk_number_from_segment_id(first_segment_id)
                print(
                    f"      📝 Keine Kontext-Paraphrasen für Dokument '{doc_name}' Chunk {chunk_num} (erste Chunks oder nicht verfügbar) (abductive)"
                )
        ...
        for r in batch_results:
            ...
            if len(relevant_categories) > 1:
                print(
                    f"   🔁 Mehrfachkodierung für Segment {r.segment_id}: {len(relevant_categories)} Kategorien über Schwellenwert {self.unified_analyzer.multiple_coding_threshold} (abductive)"
                )

                # Erste Kodierung: Verwende erste (höchste) Kategorie
                first_category = relevant_categories[0]
                first_result = self._format_single_coding_result_abductive(
                    r, coder_id, preferred_cats, seg_prefs
                )
                # Überschreibe mit erster Kategorie
        ...
        if segments_needing_multiple_coding:
            print(
                f"   🎯 Fokussierter Batch (abductive): {len(segments_needing_multiple_coding)} zusätzliche Kodierungen"
            )
            focused_results = await self._process_focused_batch(
                focused_segments=segments_needing_multiple_coding,
                category_definitions=category_definitions,
        ...
        for formatted_res in new_results:
            ...
            self.cache.set(
                analysis_mode="abductive",
                segment_text=next(
                    (s["text"] for s in batch if s["segment_id"] == cache_segment_id),
                    "",
                ),
                value=formatted_res,
                category_definitions=serializable_category_definitions,
    ...
    async def _analyze_grounded(
        ...
        **kwargs,
        ...
        print(
            f"   🔍 API Call 2: Grounded Subcode-Sammlung für {len(relevant_segments)} Segmente..."
        )
        grounded_temp = temperature or kwargs.get(
            "temperature", self.inductive_temperature
        )
        ...
        for res in results:
            ...
            segment_text = next(
                (s["text"] for s in relevant_segments if s["segment_id"] == segment_id),
                "",
            )
            ...
            if new_subcodes_in_segment:
                print(
                    f"   📝 Segment {seg_id_short}: {len(new_subcodes_in_segment)} neue Subcodes"
                )
                for subcode in new_subcodes_in_segment:
                    print(f"      • {subcode}")
            else:
                print(
                    f"   📝 Segment {seg_id_short}: Keine neuen Subcodes (bereits bekannt)"
                )
            ...
            if codes:
                for code in codes:
                    ...
                    if (
                        code_name
                        and code_confidence
                        >= self.unified_analyzer.multiple_coding_threshold
                    ):
                        relevant_categories.append(
                            {"category": code_name, "relevance_score": code_confidence}
                        )

            # Mehrfachkodierung wenn 2+ Codes über Schwellenwert
            if len(relevant_categories) > 1:
                print(
                    f"   🔁 Mehrfachkodierung für Segment {segment_id}: {len(relevant_categories)} Codes über Schwellenwert {self.unified_analyzer.multiple_coding_threshold} (grounded)"
                )
                ...
                for i, cat_info in enumerate(relevant_categories[1:], 2):
                    ...
                    print(
                        f"      📝 Mehrfachkodierung {i}: {target_category} (Score: {cat_info['relevance_score']:.2f})"
                    )
        ...
        print(
            f"   ✅ Gesamt Subcodes gesammelt: {len(self.grounded_subcodes_collection)}"
        )

        # Zeige Subcode-Zusammenfassung
        if self.grounded_subcodes_collection:
            print(f"   📋 Subcode-Übersicht:")
            for i, subcode in enumerate(
                self.grounded_subcodes_collection[:10], 1
            ):  # Zeige erste 10
            ...
            if len(self.grounded_subcodes_collection) > 10:
                print(
                    f"      ... und {len(self.grounded_subcodes_collection) - 10} weitere"
                )
    ...
    async def generate_grounded_main_categories(
        self,
        research_question: str,
        ...
        Args:
            research_question: Forschungsfrage
            initial_categories: Initiale Kategorien (optional, im Grounded Mode ungenutzt)
            max_subcategories: Max. Anzahl Subkategorien je Hauptkategorie (Default 5).
                               Die im ersten Durchlauf gesammelten Subcodes werden
                               bei Bedarf LLM-gestützt auf diesen Wert verdichtet.
        ...
        if len(self.grounded_subcodes_collection) < 5:
            print(
                f"❌ Zu wenige Subcodes für Hauptkategorien-Generierung: {len(self.grounded_subcodes_collection)} < 5"
            )
        ...
        print(
            ...
            f"(max {max_subcategories} Subkategorien je Hauptkategorie)..."
        )
        ...
        avg_confidence = (
            sum(sc.get("confidence", 0.7) for sc in subcodes_data) / len(subcodes_data)
            if subcodes_data
            else 0.7
        )
        ...
        try:
            ...
            response = await self.llm_provider.create_completion(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Du bist ein Experte für Grounded Theory. Du antwortest auf Deutsch. Antworte ausschließlich mit einem JSON-Objekt.",
                    },
            ...
            generated_categories = self._parse_main_categories_from_grounded(
                result, None
            )
            ...
            if generated_categories:
                ...
                for i, (cat_name, cat_def) in enumerate(
                    generated_categories.items(), 1
                ):
    ...
    def _build_main_categories_generation_prompt(
        self,
        ...
        Args:
            subcodes_data: Liste der gesammelten Subcodes aus Phase 1.
...
{{
  ...
  "analysis_summary": "Zusammenfassung der Kategorienbildung"
                ...
                try:
                    ...
                    if results:
                        for result in results:
                            ...
                            print(
                                f"   📝 Segment: {result.segment_id} → Kategorie: {result.primary_category}"
                            )
                            print(
                    ...
                    if results:
                        ...
                        await self._store_results_for_reliability(
                            results_for_reliability, "grounded", coder_id, coder_temp
                        )
    ...
    async def _code_single_segment_grounded(
        self,
        segment: Dict[str, Any],
        cat_defs: Dict[str, Any],
        research_question: str,
        coding_rules: List[str],
        temperature: float,
        coder_id: str,
        ...
        try:
            ...
            print(
                f"   ❌ Fehler bei Kodierung von Segment {segment['segment_id']}: {e}"
            )
    ...
    def get_optimization_metrics(self) -> Dict[str, Any]:
        ...
        optimization_metrics = {
            ...
            "avg_processing_time_per_segment_ms": summary.get(
                "avg_processing_time_per_segment_ms", 0
            ),
            "analysis_modes": {},
        }
        ...
        for mode, mode_data in summary.get("analysis_modes", {}).items():
            optimization_metrics["analysis_modes"][mode] = {
                ...
                "processing_time_ms": mode_data.get(
                    "avg_processing_time_per_segment_ms", 0
                ),
...
def create_optimization_controller(
    llm_provider, model_name: str = "gpt-4"
