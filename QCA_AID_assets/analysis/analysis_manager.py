...
from ..optimization.controller import AnalysisMode, OptimizationController
...
class IntegratedAnalysisManager:
    def __init__(self, config: Dict):
        ...
        self.research_question = config.get(
            "FORSCHUNGSFRAGE", FORSCHUNGSFRAGE
        )  # Echte Forschungsfrage aus User-Config
        self.coding_rules = config.get(
            "KODIERREGELN", KODIERREGELN
        )  # Echte Kodierregeln aus User-Config
        self.deductive_categories = config.get(
            "DEDUKTIVE_KATEGORIEN", {}
        )  # Echte Kategorien aus User-Config
        ...
        self.use_dynamic_cache = config.get(
            "ENABLE_DYNAMIC_CACHE", self.optimization_enabled
        )
        ...
        if self.optimization_enabled and not config.get("ENABLE_DYNAMIC_CACHE", True):
            print(
                "   ℹ️ ENABLE_OPTIMIZATION=True aktiviert automatisch ENABLE_DYNAMIC_CACHE=True"
            )
        ...
        if self.optimization_enabled:
            try:
                ...
                print(
                    f"   🚀 OptimizationController initialisiert (Batching & Caching aktiviert)"
                )
                ...
                if self.use_dynamic_cache:
                    try:
                        ...
                        self.dynamic_cache_manager = (
                            self.optimization_controller.dynamic_cache_manager
                        )

                        print(
                            f"   🗄️ DynamicCacheManager vom OptimizationController wiederverwendet"
                        )
                        print(
                            f"   📊 Reliability Database: {self.dynamic_cache_manager.reliability_db.db_path}"
                        )

                    except Exception as e:
                        print(
                            f"   ⚠️ Konnte DynamicCacheManager nicht wiederverwenden: {e}"
                        )
        ...
        print(
            f"   - Optimization: {'✅ Aktiviert' if self.optimization_enabled else '❌ Deaktiviert'}"
        )
        print(
            f"   - Dynamic Cache: {'✅ Aktiviert' if self.use_dynamic_cache else '❌ Deaktiviert (Legacy Mode)'}"
        )
        print(
            f"   - Reliability: {'🗄️ Database' if self.use_dynamic_cache else '📊 Legacy Calculator'}"
        )
    ...
    async def _get_next_batch(
        self, segments: List[Tuple[str, str]], batch_size: float
    ) -> List[Tuple[str, str]]:
        """
        ...
        """
        remaining_segments = [
            seg for seg in segments if seg[0] not in self.processed_segments
        ]
    ...
    async def _process_batch_inductively(
        self,
        ...
        if analysis_mode == "inductive":
            return await self._process_inductive_mode(
                relevant_segments, current_categories
            )
        elif analysis_mode == "abductive":
            return await self._process_abductive_mode(
                relevant_segments, current_categories
            )
        elif analysis_mode == "grounded":
            return await self._process_grounded_mode(
                relevant_segments, current_categories
            )
        else:
            return {}

    async def _process_inductive_mode(
        self,
    ...
    async def _process_abductive_mode(
        self,
        relevant_segments: List[str],
        current_categories: Dict[str, CategoryDefinition],
    ) -> Dict[str, CategoryDefinition]:
    ...
    async def _process_grounded_mode(
        self,
        relevant_segments: List[str],
        current_categories: Dict[str, CategoryDefinition],
    ) -> Dict[str, CategoryDefinition]:
        ...
        """
        print(
            "❌ WARNUNG: _process_grounded_mode sollte nicht in separatem Grounded Mode aufgerufen werden!"
        )
        return {}

    async def _assess_grounded_saturation(
        self, batch_count: int, total_batches: int
    ) -> bool:
        """
        ...
        try:
            ...
            if hasattr(self, "chunks") and self.chunks:
                total_segments = sum(
                    len(chunk_list) for chunk_list in self.chunks.values()
                )
            ...
            material_percentage = (
                (len(self.processed_segments) / total_segments) * 100
                if total_segments > 0
                else 0.0
            )
            ...
            subcode_diversity = len(
                set(sc["name"] for sc in self.grounded_subcodes_collection)
            )
            ...
            avg_subcodes_per_batch = len(self.grounded_subcodes_collection) / max(
                batch_count, 1
            )
            ...
            criteria = {
                ...
                "material_coverage": material_percentage
                >= 70,  # 70% Material verarbeitet
                "subcodes_collected": len(self.grounded_subcodes_collection)
                >= 8,  # Min. 8 Subcodes
                "saturation_stability": self.grounded_saturation_counter
                >= 2,  # 2 Batches ohne neue
                "diversity_threshold": subcode_diversity
                >= 5,  # Mindestens 5 verschiedene Subcodes
                "keyword_richness": keyword_diversity
                >= 15,  # Mindestens 15 verschiedene Keywords
            }
            ...
            partial_saturation = critical_met and (
                material_percentage >= 85 or criteria["material_coverage"]
            )
            forced_saturation = (
                material_percentage >= 100
            )  # 100% Material = ZwangssÄttigung
            ...
            if is_saturated:
                saturation_type = (
                    "Vollständig"
                    if full_saturation
                    else ("Partiell" if partial_saturation else "Material-bedingt")
                )
                print(f"\n🎯 GROUNDED MODE SÄTTIGUNG erreicht ({saturation_type}):")
                print(f"   - Material: {material_percentage:.1f}% verarbeitet")
                print(
                    f"   - Subcodes: {len(self.grounded_subcodes_collection)} gesammelt"
                )
                print(f"   - Sättigungs-Counter: {self.grounded_saturation_counter}")
            else:
                print(
    ...
    async def _analyze_for_subcategories(
        self,
        relevant_segments: List[str],
        current_categories: Dict[str, CategoryDefinition],
    ) -> Dict[str, CategoryDefinition]:
        ...
        """
        segments_text = "\n\n=== SEGMENT BREAK ===\n\n".join(
            f"SEGMENT {i + 1}:\n{text}" for i, text in enumerate(relevant_segments)
        )
        ...
        prompt = self.prompt_handler.get_analyze_for_subcategories_prompt(
            segments_text=segments_text, categories_context=categories_context
        )
        ...
        try:
            ...
            response = await self.inductive_coder.llm_provider.create_completion(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Du bist ein Experte fuer qualitative Inhaltsanalyse. Antworte auf deutsch.",
                    },
            ...
            for main_cat_name, updates in result.get("extended_categories", {}).items():
                if main_cat_name in extended_categories:
                    ...
                    for sub_data in updates.get("new_subcategories", []):
                        if (
                            sub_data.get("confidence", 0) >= 0.7
                        ):  # Schwelle fuer Subkategorien
                            new_subcats[sub_data["name"]] = sub_data["definition"]
                            total_new_subcats += 1
                            print(
                                f"✅ Neue Subkategorie: {main_cat_name} -> {sub_data['name']}"
                            )
    ...
    async def _code_batch_deductively(
        self,
        batch: List[Tuple[str, str]],
        categories: Dict[str, CategoryDefinition],
        category_preselections: Dict[str, Dict] = None,
    ) -> List[Dict]:
        """
        ...
        if self.use_context and batch:
            ...
            if context_paraphrases:
                print(
                    f"📝 Nutze {len(context_paraphrases)} Kontext-Paraphrasen für Dokument '{doc_name}'"
                )
                # DEBUG: Zeige erste Paraphrase als Beispiel
                if context_paraphrases:
                    first_paraphrase = (
                        context_paraphrases[0][:100] + "..."
                        if len(context_paraphrases[0]) > 100
                        else context_paraphrases[0]
                    )
                    print(f"   📝 Beispiel-Kontext: {first_paraphrase}")
            else:
                print(
                    f"📝 Keine Kontext-Paraphrasen für Dokument '{doc_name}' verfügbar (erste Chunks oder leer)"
                )
        else:
            context_paraphrases = []
        ...
        if category_preselections:
            preselected_count = len(
                [s for s in batch if s[0] in category_preselections]
            )
        ...
        coding_relevance_results = await self.relevance_checker.check_relevance_batch(
            batch
        )
        ...
        relevant_count = sum(
            1 for is_relevant in coding_relevance_results.values() if is_relevant
        )
        print(f"   - Segmente geprÜft: {len(coding_relevance_results)}")
        print(f"   - Als kodierungsrelevant eingestuft: {relevant_count}")
        print(
            f"   - Als nicht kodierungsrelevant eingestuft: {len(coding_relevance_results) - relevant_count}"
        )
        ...
        if CONFIG.get("MULTIPLE_CODINGS", True):
            ...
            if coding_relevant_segments:
                print(
                    f"  ℹ️ Prüfe {len(coding_relevant_segments)} kodierungsrelevante Segmente auf Mehrfachkodierung..."
                )
                multiple_coding_results = (
                    await self.relevance_checker.check_multiple_category_relevance(
                        coding_relevant_segments, categories
                    )
                )

        # 3. PARALLEL: Kodierung aller Segmente
        async def code_single_segment_all_coders(
            ...
            is_coding_relevant = coding_relevance_results.get(
                segment_id, True
            )  # Default: True
            ...
            if not is_coding_relevant:
                ...
                if len(text.strip()) < 20:
                    justification = (
                        "Segment zu kurz fuer sinnvolle Kodierung (<20 Zeichen)"
                    )
                elif is_metadata:
                ...
                relevance_details = self.relevance_checker.get_relevance_details(
                    segment_id
                )
                if relevance_details:
                    if (
                        "reasoning" in relevance_details
                        and relevance_details["reasoning"]
                        and "Keine Begründung" not in relevance_details["reasoning"]
                    ):
                        justification = relevance_details["reasoning"]
                    elif (
                        "justification" in relevance_details
                        and relevance_details["justification"]
                    ):
                ...
                for coder in self.deductive_coders:
                    result = {
                        ...
                        "context_paraphrases_used": bool(
                            context_paraphrases
                        ),  # NEU: Ob Kontext-Paraphrasen tatsächlich verfügbar waren
                    }
            ...
            if preferred_cats:
                ...
                effective_categories = {
                    ...
                    )  # FIX: Validiere CategoryDefinition
                }

                if not effective_categories:
                    print(
                        f"    ❌ Keine gültigen CategoryDefinition-Objekte in preferred_cats - verwende alle Kategorien"
                    )
                    effective_categories = categories
                else:
                    print(
                        f"    🎯 Segment {segment_id}: Fokus auf {len(effective_categories)} Kategorien: {', '.join(preferred_cats)}"
                    )
                    ...
                    for name, cat in effective_categories.items():
                        if not hasattr(cat, "subcategories"):
                            print(
                                f"    ❌ KRITISCH: effective_categories['{name}'] fehlen Subkategorien - hole aus categories"
                            )
                ...
                effective_categories = categories
                print(
                    f"    🔀 Segment {segment_id}: Standard-Kodierung mit allen {len(categories)} Kategorien"
                )
            ...
            async def code_with_coder_and_instance(coder, instance_info):
                ...
                try:
                    ...
                    if target_cat:
                        if target_cat and target_cat not in enhanced_categories:
                            if target_cat in categories:
                                ...
                                print(
                                    f"    🎯 Fokuskategorie '{target_cat}' zu verfügbaren Kategorien hinzugefÜgt"
                                )
                            else:
                                print(
                                    f"    ❌ Fokuskategorie '{target_cat}' nicht in Kategorien vorhanden"
                                )
                    ...
                    if coding and isinstance(coding, CodingResult):
                        ...
                        if main_category in enhanced_categories and hasattr(
                            enhanced_categories[main_category], "subcategories"
                        ):
                            try:
                                ...
                                print(
                                    f"    ⚠️ Validierung mit enhanced_categories fehlgeschlagen: {str(e)}"
                                )
                                # FIX: Fallback zu self.current_categories
                                if (
                                    hasattr(self, "current_categories")
                                    and main_category in self.current_categories
                                ):
                                    try:
                                        validated_subcats = CategoryValidator.validate_subcategories_for_category(
                                            original_subcats,
                                        ...
                                        validation_source = (
                                            "self.current_categories_fallback"
                                        )
                                    except Exception as e2:
                                        print(
                                            f"    ⚠️ Auch Fallback-Validierung fehlgeschlagen: {str(e2)}"
                                        )
                        else:
                            # FIX: Informative Meldung bei nicht verfügbaren Kategorien
                            if main_category not in enhanced_categories:
                                print(
                                    f"    ℹ️ Kategorie '{main_category}' nicht in verfügbaren Kategorien"
                                )
                                print(
                                    f"    🎯 verfügbare Kategorien: {list(enhanced_categories.keys())}"
                                )
                            elif not hasattr(
                                enhanced_categories[main_category], "subcategories"
                            ):
                                print(
                                    f"    ℹ️ Keine Subkategorie-Definitionen verfügbar fuer '{main_category}'"
                                )
                        ...
                        if len(original_subcats) != len(validated_subcats):
                            ...
                            print(
                                f"    🔧 Subkategorien bereinigt: {len(original_subcats)} -> {len(validated_subcats)}"
                            )
                            if removed:
                                print(
                                    f"    🔧 Entfernt: {list(removed)} (Quelle: {validation_source})"
                                )
                        elif validation_source != "keine" and original_subcats:
                            print(
                                f"    ✅ Alle {len(original_subcats)} Subkategorien gültig (Quelle: {validation_source})"
                            )
                        elif validation_source == "keine" and original_subcats:
                            print(
                                f"    ℹ️ Subkategorien-Validierung Übersprungen fuer '{main_category}' (Quelle: {validation_source})"
                            )
                        ...
                        return {
                            ...
                            "category_focus_used": bool(
                                instance_info["target_category"]
                            ),
                            # FIX: Verbesserte Debug-Informationen fuer enhanced_categories
                            "category_preselection_used": bool(preferred_cats),
                            "preferred_categories": preferred_cats,
                            "effective_categories_count": len(effective_categories),
                            "enhanced_categories_count": len(
                                enhanced_categories
                            ),  # FIX: Neue Info
                            "preselection_reasoning": preselection.get("reasoning", ""),
                            "subcategories_validated": len(original_subcats)
                            != len(validated_subcats),
                            "validation_source": validation_source,
                            "validation_successful": validation_source != "keine",
                            "category_in_enhanced": main_category
                            in enhanced_categories,  # FIX: Verwende enhanced_categories
                            "enhanced_has_subcategories": main_category
                            in enhanced_categories
                            and hasattr(
                                enhanced_categories[main_category], "subcategories"
                            ),  # FIX: Neue Debug-Info
                            "focus_category_added": instance_info["target_category"]
                            and instance_info["target_category"]
                            not in effective_categories,  # FIX: Neue Info
                            "context_paraphrases_used": bool(
                                context_paraphrases
                            ),  # NEU: Ob Kontext-Paraphrasen tatsächlich verfügbar waren
                        }
        ...
        all_segment_results = await asyncio.gather(
        ...
        if self.use_context and batch_results:
            for result in batch_results:
                if (
                    ...
                    and result.get("category") != "Nicht kodiert"
                ):
            ...
            if batch:
                ...
                print(
                    f"📝 Dokument '{doc_name}': {total_paraphrases} Paraphrasen gesammelt (davon werden max. {self.context_paraphrase_count} als Kontext genutzt)"
                )
        ...
        if processing_time > 0:
            print(
                f"   🚀 Geschwindigkeit: {len(batch) / processing_time:.1f} Segmente/Sekunde"
            )
        else:
            print(
                f"   🚀 Geschwindigkeit: {len(batch)} Segmente in <0.01s (sehr schnell)"
            )
        ...
        if category_preselections:
            print(
                f"   🎯 Kategorie-Vorauswahl genutzt: {preselection_used_count} Kodierungen"
            )
            print(
                f"   🔧 Subkategorie-Validierung durchgefÜhrt: {validation_performed_count} Kodierungen"
            )
        if error_count > 0:
    ...
    def _collect_existing_paraphrases_for_context(
        ...
        for segment in segments:
            ...
            if doc_name in self.document_paraphrases:
                collected_paraphrases[doc_name].extend(
                    self.document_paraphrases[doc_name]
                )

            # 2. Aus dem Cache (falls verfügbar)
            if (
                hasattr(self, "optimization_controller")
                and self.optimization_controller
            ):
                try:
                    # Versuche aus dem Cache zu lesen
                    cache_key = f"coding_{segment_id}"
                    cached_result = self.optimization_controller.cache.get_by_key(
                        cache_key
                    )
                    if cached_result and isinstance(cached_result, dict):
                        ...
                        if (
                            cached_paraphrase
                            and cached_paraphrase not in collected_paraphrases[doc_name]
                        ):
            ...
            if hasattr(self, "coding_results") and self.coding_results:
                for result in self.coding_results:
                    if result.get("segment_id") == segment_id:
                        ...
                        if (
                            result_paraphrase
                            and result_paraphrase not in collected_paraphrases[doc_name]
                        ):
    ...
    def _collect_paraphrase_for_context(self, segment_id: str, paraphrase: str) -> None:
        ...
        if paraphrase not in self.document_paraphrases[doc_name]:
            self.document_paraphrases[doc_name].append(paraphrase)
            print(
                f"      📝 ➕ Paraphrase gesammelt für {doc_name}: {len(self.document_paraphrases[doc_name])} verfügbar"
            )
    ...
    async def _finalize_by_mode(
        self,
        analysis_mode: str,
        current_categories: Dict,
        deductive_categories: Dict,
        ...
        try:
            if analysis_mode == "inductive":
                ...
                print(
                    f"   -> Finales System: NUR {len(current_categories)} induktive Kategorien"
                )
                return current_categories
                ...
                print(
                    f"   - Erweiterte deduktive Kategorien: {len(current_categories)}"
                )
                return current_categories
    ...
    def _show_final_development_stats(
        self, final_categories: Dict, initial_categories: Dict, batch_count: int
    ):
        """
        ...
        if analysis_mode == "inductive":
            ...
            total_subcats = sum(
                len(cat.subcategories) for cat in final_categories.values()
            )
            ...
            initial_count = (
                len(initial_categories) if initial_categories else 0
            )  # ✅ BUGFIX: len() hinzugefÜgt
        ...
        if (
            hasattr(self, "inductive_coder")
            and self.inductive_coder
            and hasattr(self.inductive_coder, "theoretical_saturation_history")
            and self.inductive_coder.theoretical_saturation_history
        ):
            final_saturation = self.inductive_coder.theoretical_saturation_history[-1]
            print(f"\n🎯 Finale Sättigung:")
            print(
                f"   - Theoretische Sättigung: {final_saturation['theoretical_saturation']:.1%}"
            )
            print(
                f"   - Kategorienqualität: {final_saturation['category_quality']:.1%}"
            )
        ...
        if (
            hasattr(self, "inductive_coder")
            and self.inductive_coder
            and hasattr(self.inductive_coder, "category_development_phases")
            and self.inductive_coder.category_development_phases
        ):
            print(f"\n🧾 Entwicklungsphasen:")
            for phase in self.inductive_coder.category_development_phases:
                print(
                    f"   Batch {phase['batch']}: +{phase['new_categories']} -> {phase['total_categories']} total"
                )
    ...
    async def _save_grounded_checkpoint(self):
        ...
        try:
            checkpoint_path = os.path.join(
                self.config["OUTPUT_DIR"], "grounded_checkpoint.json"
            )
            checkpoint_data = {
            ...
            print(
                f"ðŸ’¾ Grounded Checkpoint gespeichert: {len(self.grounded_subcodes_collection)} Subcodes"
            )
    ...
    async def _load_grounded_checkpoint(self):
        ...
        try:
            ...
            if os.path.exists(checkpoint_path):
                ...
                self.grounded_saturation_counter = checkpoint_data.get(
                    "saturation_counter", 0
                )

                print(
                    f"ðŸ’¾ Grounded Checkpoint geladen: {len(self.grounded_subcodes_collection)} Subcodes"
                )
                print(f"   - Keywords: {len(self.grounded_keywords_collection)}")
                print(
                    f"   - Batch-Historie: {len(self.grounded_batch_history)} Einträge"
                )
                return True
        except Exception as e:
    ...
    async def analyze_material(
        ...
        try:
            ...
            print(
                f"Verarbeite {total_segments} Segmente mit Batch-Gröẞe {batch_size}..."
            )
    ...
    async def _analyze_normal_modes(
        self,
        chunks: Dict[str, List[str]],
        initial_categories: Dict,
        all_segments: List,
        ...
        if self.optimization_enabled and self.optimization_controller:
            ...
            if analysis_mode not in supported_modes:
                print(
                    f"⚠️ Modus '{analysis_mode}' wird im OptimizationController nicht unterstützt, verwende Standard-Analyse"
                )
            else:
                print(
                ...
                opt_segments = [
                    {"segment_id": s[0], "text": s[1]} for s in all_segments
                ]

                # Mapping Coding Rules - verwende echte Kodierregeln aus User-Config
                rules = self.coding_rules.get("general", []) + self.coding_rules.get(
                    "format", []
                )
                ...
                try:
                    ...
                    print(
                        f"   🚀 Starte optimierte Analyse für {len(opt_segments)} Segmente..."
                    )
                    print(
                        f"   🔍 DEBUG: DynamicCacheManager verfügbar: {self.dynamic_cache_manager is not None}"
                    )
                    if self.dynamic_cache_manager:
                        print(
                            f"   🔍 DEBUG: Reliability DB path: {self.dynamic_cache_manager.reliability_db.db_path}"
                        )
                    ...
                    if self.use_context:
                        print(
                            f"   📝 Progressive Paraphrasen-Sammlung aktiviert (Callback-System)"
                        )
                    ...
                    (
                        ...
                        paraphrase_callback=self._collect_paraphrase_for_context
                        if self.use_context
                        else None,
                    )

                    print(
                        f"✅ Optimierte Analyse abgeschlossen: {len(results)} Ergebnisse für {len(opt_segments)} Segmente"
                    )
                    ...
                    if self.dynamic_cache_manager:
                        try:
                            print(
                                f"   💾 DEBUG: Versuche {len(getattr(self.dynamic_cache_manager, 'pending_results', []))} Kodierungen zu speichern..."
                            )
                            ...
                            logger.error(
                                f"Fehler beim Speichern der gesammelten Kodierungen: {e}"
                            )
                            raise

                    print(
                        f"   🔍 DEBUG: Checking reliability database after optimization..."
                    )
                    if self.dynamic_cache_manager:
                        try:
                            db_results = (
                                self.dynamic_cache_manager.get_all_coding_data()
                            )
                            print(
                                f"   🔍 DEBUG: Reliability DB now contains {len(db_results)} total results"
                            )
                            relevant_results = (
                                self.dynamic_cache_manager.get_reliability_data(
                                    exclude_non_relevant=True
                                )
                            )
                            print(
                                f"   🔍 DEBUG: Of which {len(relevant_results)} are relevant for reliability"
                            )
                        except Exception as e:
                            print(f"   ❌ DEBUG: Error checking reliability DB: {e}")

                    # Update current_categories with any developed/extended categories
                    if updated_categories and analysis_mode in [
                        "inductive",
                        "abductive",
                    ]:
                        print(
                            f"   🔄 Aktualisiere Kategorien: {len(current_categories)} → {len(updated_categories)}"
                        )
                    ...
                    for opt_result in results:
                        ...
                        if "result" in opt_result:
                            ...
                            primary_category = opt_result.get(
                                "primary_category", "Nicht kodiert"
                            )
                        ...
                        coding_result = {
                            ...
                            "confidence": {
                                ...
                                else confidence_val.get("total", 0.7),
                                "category": confidence_val
                                if isinstance(confidence_val, (int, float))
                                else confidence_val.get("category", 0.7),
                                "subcategories": 1.0,
                            },
                            "justification": justification
                            if justification
                            else f"Confidence: {confidence_val:.2f}",
                            "text": segment_text,
                            ...
                            "context_paraphrases_used": self._should_have_context_paraphrases(
                                segment_id
                            ),  # NEU: Korrekte Kontext-Erkennung basierend auf Segment-Position
                        }
                    ...
                    if self.use_context and self.document_paraphrases:
                        for doc_name, paraphrases in self.document_paraphrases.items():
                            print(
                                f"📝 Dokument '{doc_name}': {len(paraphrases)} Paraphrasen gesammelt (davon werden max. {self.context_paraphrase_count} als Kontext genutzt)"
                            )
                    ...
                    total_tokens = final_stats.get("input", 0) + final_stats.get(
                        "output", 0
                    )
                    ...
                    unique_coders = set(
                        c.get("coder_id", "auto_1") for c in converted_results
                    )
        ...
        while True:
            ...
            if self.check_escape_abort():
                ...
                await self._export_intermediate_results(
                    chunks, current_categories, deductive_categories, initial_categories
                )
                return current_categories, self.coding_results
            ...
            try:
                ...
                if analysis_mode == "deductive":
                    ...
                    for segment_id, text in batch:
                        ...
                        if result.get("is_relevant", False):
                            ...
                            category_preselections[segment_id] = {
                                "preferred_categories": result.get(
                                    "preferred_categories", []
                                ),
                    ...
                    print(
                        f"🧾 Erweiterte Relevanz: {len(generally_relevant_batch)} von {len(batch)} Segmenten relevant"
                    )
                    if category_preselections:
                        preselection_stats = {}
                        for prefs in category_preselections.values():
                            for cat in prefs["preferred_categories"]:
                                preselection_stats[cat] = (
                                    preselection_stats.get(cat, 0) + 1
                                )
                        print(f"🎯 Kategorie-Präferenzen: {preselection_stats}")

                else:
                    # FIX: Standard-Relevanzprüfung fuer andere Modi (unverÄndert)
                    general_relevance_results = (
                        await self.relevance_checker.check_relevance_batch(batch)
                    )
                    generally_relevant_batch = [
                        (segment_id, text)
                        for segment_id, text in batch
                    ...
                    print(
                        f"🧾 Allgemeine Relevanz: {len(generally_relevant_batch)} von {len(batch)} Segmente relevant"
                    )
                ...
                if not skip_inductive and generally_relevant_batch:
                    ...
                    if analysis_mode in ["inductive", "abductive"]:
                        ...
                        if new_categories:
                            ...
                            after_subcategories = sum(
                                len(cat.subcategories)
                                for cat in current_categories.values()
                            )
                            added_subcategories = (
                                after_subcategories - before_subcategories
                            )
                            ...
                            if analysis_mode == "abductive":
                                if added_count > 0:
                                    print(
                                        f"✅ {added_count} neue Hauptkategorien integriert"
                                    )
                                if added_subcategories > 0:
                                    print(
                                        f"✅ {added_subcategories} neue Subkategorien integriert"
                                    )
                                if added_count == 0 and added_subcategories == 0:
                                    print(
                                        "✅ 0 neue Kategorien integriert (wie erwartet im abduktiven Modus)"
                                    )
                            else:
                                print(f"✅ {added_count} neue Kategorien integriert")
                                if added_subcategories > 0:
                                    print(
                                        f"   🔀 Zusätzlich {added_subcategories} neue Subkategorien"
                                    )
                ...
                if analysis_mode == "inductive":
                    if len(current_categories) == 0:
                        coding_categories = {}
                        print(
                            f"   🔀 Inductive Mode: Keine induktiven Kategorien -> 'Nicht kodiert'"
                        )
                    else:
                        coding_categories = current_categories
                        print(
                            f"   🔀 Inductive Mode: Verwende {len(current_categories)} induktive Kategorien"
                        )
                elif analysis_mode == "grounded":
                    ...
                    print(
                        f"   🔀 Grounded Mode: Verwende {len(grounded_categories)} rein induktive Kategorien"
                    )
                    print(
                        f"   ❌ Ausgeschlossen: {len(current_categories) - len(grounded_categories)} deduktive Kategorien"
                    )
                else:
                    coding_categories = current_categories
                ...
                material_percentage = (
                    len(self.processed_segments) / total_segments
                ) * 100
                ...
                if analysis_mode in ["inductive", "grounded"]:
                    ...
                    print(f"\n🧾 Sättigungsstatus:")
                    print(
                        f"   🎯 Theoretische Sättigung: {saturation_status['theoretical_saturation']:.1%}"
                    )
                    print(
                        f"   ℹ️ Materialabdeckung: {saturation_status['material_coverage']:.1%}"
                    )
                ...
                print(f"\nℹ️ Fortschritt:")
                print(
                    f"   - Verarbeitete Segmente: {len(self.processed_segments)}/{total_segments}"
                )
        ...
        self._show_final_development_stats(
            final_categories, initial_categories, batch_count
        )

        # KORRIGIERT: Stelle sicher, dass immer ein Tupel zurÜckgegeben wird
        return final_categories, self.coding_results

    async def _analyze_grounded_mode(
        self,
        chunks: Dict[str, List[str]],
        initial_categories: Dict,
        all_segments: List,
        ...
        if not self.optimization_controller:
            print(
                "⚠️ OptimizationController nicht verfügbar - verwende Standard-Implementierung"
            )
            return await self._analyze_grounded_mode_standard(
                chunks, initial_categories, all_segments, batch_size
            )
        ...
        try:
            ...
            if len(self.optimization_controller.grounded_subcodes_collection) >= 5:
                ...
                max_subcats = CONFIG.get("MAX_SUBCATEGORIES", 5)
                try:
                    max_subcats = int(max_subcats)
                except (TypeError, ValueError):
                    max_subcats = 5
                max_subcats = max(1, min(max_subcats, 50))
                ...
                if grounded_categories:
                    ...
                    for opt_result in coding_results:
                        ...
                        coding_result = {
                            ...
                            "justification": justification
                            if justification
                            else f"Grounded Analysis: Confidence {confidence_val:.2f}",
                            "text": segment_text,
                            "paraphrase": paraphrase if paraphrase else "",
                            "keywords": keywords if keywords else "",
                            ...
                            "category_focus_used": False,
                            "category_preselection_used": result_data.get(
                                "category_preselection_used", False
                            )
                            if "result" in opt_result
                            else False,
                            "preferred_categories": result_data.get(
                                "preferred_categories", []
                            )
                            if "result" in opt_result
                            else [],
                    ...
                    print(
                        "⚠️ Keine Hauptkategorien generiert - verwende Subcodes als Kategorien"
                    )
                ...
                print(
                    f"⚠️ Zu wenige Subcodes für Hauptkategorien-Generierung: {len(self.optimization_controller.grounded_subcodes_collection)} < 5"
                )
                ...
                for opt_result in subcode_results:
                    ...
                    if "result" in opt_result:
                        ...
                        paraphrase = opt_result.get("memo", "")
                        justification = (
                            f"Subcode Analysis: {len(subcategories)} codes identified"
                        )
                        coder_id = "auto_1"
    ...
    async def _analyze_grounded_mode_standard(
        self,
        chunks: Dict[str, List[str]],
        initial_categories: Dict,
        all_segments: List,
        ...
        while True:
            ...
            batch_count += 1
            material_percentage = (
                len(self.processed_segments) / len(all_segments)
            ) * 100

            print(f"\n{'=' * 60}")
            print(
                f"🧾 GROUNDED BATCH {batch_count}: {len(batch)} Segmente (NUR SUBCODE-SAMMLUNG)"
            )
            print(f"ℹ️ Material verarbeitet: {material_percentage:.1f}%")
            print(f"{'=' * 60}")
            ...
            if generally_relevant_batch:
                ...
                grounded_analysis = await self.inductive_coder.analyze_grounded_batch(
                    segments=relevant_texts, material_percentage=material_percentage
                )
            ...
            if await self._assess_grounded_saturation(
                batch_count, len(all_segments) / batch_size
            ):
                print(
        ...
        if len(self.grounded_subcodes_collection) >= 5:
            ...
            self.inductive_coder.grounded_segment_analyses = (
                self.grounded_segment_analyses
            )

            # Generiere Hauptkategorien
            grounded_categories = (
                await self.inductive_coder._generate_main_categories_from_subcodes(
                    initial_categories
                )
            )
            ...
            if grounded_categories:
                ...
                print(
                    "⚠️ Keine Hauptkategorien generiert - verwende initiale Kategorien"
                )
                return initial_categories, []
        else:
            print(
                f"❌ Zu wenige Subcodes: {len(self.grounded_subcodes_collection)} < 5"
            )
            return initial_categories, []
    ...
    def _collect_grounded_subcodes(self, grounded_analysis: Dict, batch_number: int):
        ...
        if grounded_analysis and "segment_analyses" in grounded_analysis:
            print(
                f"🔀 Verarbeite {len(grounded_analysis['segment_analyses'])} Segment-Analysen"
            )
            ...
            for segment_analysis in grounded_analysis["segment_analyses"]:
                ...
                for subcode in subcodes:
                    ...
                    if subcode_name:
                        # Prüfe auf Duplikate
                        existing_names = [
                            sc["name"] for sc in self.grounded_subcodes_collection
                        ]
                        ...
                        if subcode_name not in existing_names:
                            ...
                            # Sammle Keywords
                            self.grounded_keywords_collection.extend(
                                subcode.get("keywords", [])
                            )
                            ...
                            for existing_subcode in self.grounded_subcodes_collection:
                                if existing_subcode["name"] == subcode_name:
                                    ...
                                    existing_keywords = set(
                                        existing_subcode["keywords"]
                                    )
                                    new_keywords = set(subcode.get("keywords", []))
                                    existing_subcode["keywords"] = list(
                                        existing_keywords | new_keywords
                                    )

                                    existing_subcode["evidence"].extend(
                                        subcode.get("evidence", [])
                                    )
        ...
        batch_info = {
            ...
            "material_percentage": (len(self.processed_segments) / self._total_segments)
            * 100,
        }
    ...
    async def _code_all_segments_with_grounded_categories(
        self, all_segments: List, grounded_categories: Dict, use_context: bool
    ) -> List[Dict]:
        """
        NEUE METHODE: Kodiere alle Segmente mit den generierten Grounded-Kategorien
        """
        print(
            f"📝 Kodiere {len(all_segments)} Segmente mit {len(grounded_categories)} Grounded-Kategorien"
        )
    ...
    async def _recode_segments_with_final_categories(
        self,
        final_categories: Dict[str, CategoryDefinition],
        chunks: Dict[str, List[str]],
    ) -> None:
        """
        ...
        print(
            f"🧾 Kodiere {len(all_segments_to_recode)} Segmente mit Grounded-Kategorien"
        )
        ...
        for i in range(0, len(all_segments_to_recode), batch_size):
            ...
            for segment_id, segment_text in batch:
                try:
                    ...
                    coding_result = await self.deductive_coders[0].code_chunk(
                        segment_text, final_categories
                    )
        ...
        if new_codings:
            print(
                f"ℹ️ Ersetze {len(self.coding_results)} alte durch {len(new_codings)} neue Kodierungen"
            )
            ...
            category_dist = Counter(
                coding.get("category", "Unbekannt") for coding in new_codings
            )
    ...
    def _show_grounded_mode_statistics(self):
        ...
        if self.collected_subcodes:
            ...
            print(
                f"   - Top Keywords: {', '.join([f'{kw}({count})' for kw, count in top_keywords[:5]])}"
            )

            # Subcode-Batch-Verteilung
            batch_dist = Counter(
                sc.get("batch_number", 0) for sc in self.collected_subcodes
            )
    ...
    def _export_grounded_mode_details(self, output_dir: str):
        ...
        try:
            ...
            subcodes_path = os.path.join(
                output_dir,
                f"grounded_subcodes_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
            )
            ...
            export_data = {
                ...
                "segment_analyses": self.grounded_segment_analyses[
                    :100
                ],  # Nur erste 100 fuer Gröẞe
            }
    ...
    def check_escape_abort(self) -> bool:
        ...
        return (
            getattr(self, "_should_abort", False)
            or getattr(self, "_escape_abort_requested", False)
            or (hasattr(self, "escape_handler") and self.escape_handler.should_abort())
        )

    async def _export_intermediate_results(
        self, chunks, current_categories, deductive_categories, initial_categories
    ):
        ...
        try:
            ...
            if current_categories:
                ...
                print(
                    f"🔀 Zwischenkategorien gespeichert: {len(current_categories)} Kategorien"
                )

            # Exportiere Zwischenkodierungen falls vorhanden
            if self.coding_results:
                print(
                    f"🧾 Exportiere {len(self.coding_results)} Zwischenkodierungen..."
                )

                # Revision Manager fuer Export
                revision_manager = CategoryRevisionManager(
                    output_dir=CONFIG["OUTPUT_DIR"], config=CONFIG
                )
    ...
    def _merge_category_systems(
        self, current: Dict[str, CategoryDefinition], new: Dict[str, CategoryDefinition]
    ) -> Dict[str, CategoryDefinition]:
        """
        ...
        for name, category in new.items():
            if name not in merged:
                ...
                if len(new_examples) > len(existing.examples):
                    changes.append(
                        f"{len(new_examples) - len(existing.examples)} neue Beispiele"
                    )
                ...
                if len(new_rules) > len(existing.rules):
                    changes.append(
                        f"{len(new_rules) - len(existing.rules)} neue Regeln"
                    )
                ...
                if len(new_subcats) > len(existing.subcategories):
                    changes.append(
                        f"{len(new_subcats) - len(existing.subcategories)} neue Subkategorien"
                    )
    ...
    def _find_similar_category(
        ...
        try:
            ...
            for existing_name, existing_cat in existing_categories.items():
                ...
                name_similarity = (
                    self.inductive_coder._calculate_text_similarity(
                        category.name.lower(), existing_name.lower()
                    )
                    * 0.3
                )
                ...
                definition_similarity = (
                    self.inductive_coder._calculate_text_similarity(
                        category.definition, existing_cat.definition
                    )
                    * 0.5
                )
                ...
                # GesamtÄhnlichkeit
                total_similarity = (
                    name_similarity + definition_similarity + subcat_similarity
                )
    ...
    def _update_tracking(
        self, batch_results: List[Dict], processing_time: float, batch_size: int
    ) -> None:
        """
    ...
    def _log_iteration_status(
        self, material_percentage: float, saturation_metrics: Dict, num_results: int
    ) -> None:
        """
        ...
        try:
            ...
            status = {
                ...
                "processing_time": self.performance_metrics["batch_processing_times"][
                    -1
                ]
    ...
    def _finalize_analysis(
        self, final_categories: Dict, initial_categories: Dict
    ) -> Tuple[Dict, List]:
        """
    ...
    def get_analysis_report(self) -> Dict:
        ...
        return {
            ...
            "performance_metrics": {
                ...
                "total_batches": len(
                    self.performance_metrics["batch_processing_times"]
                ),
    ...
    def _get_coding_summary(self) -> Dict:
        ...
        return {
            ...
            "avg_codings_per_segment": len(self.coding_results)
            / len(self.processed_segments)
    ...
    def get_progress_report(self) -> Dict:
        ...
        avg_batch_time = (
            statistics.mean(self.performance_metrics["batch_processing_times"])
            if self.performance_metrics["batch_processing_times"]
            else 0
        )
        avg_coding_time = (
            statistics.mean(self.performance_metrics["coding_times"])
            if self.performance_metrics["coding_times"]
            else 0
        )
        ...
        if hasattr(self, "consolidated_results") and self.consolidated_results:
            ...
            coding_info = (
                f"{total_codings} ursprünglich, {display_codings} konsolidiert"
            )
        else:
        ...
        return {
            ...
            "performance": {
                ...
                "last_batch_time": round(
                    self.performance_metrics["batch_processing_times"][-1], 2
                )
                if self.performance_metrics["batch_processing_times"]
                else 0,
            },
    ...
    async def _code_batch_with_context(
        self, batch: List[Tuple[str, str]], categories: Dict[str, CategoryDefinition]
    ) -> List[Dict]:
        """
    ...
    async def _store_batch_results_for_reliability(
    ...
    def get_reliability_data(self, exclude_non_relevant: bool = True) -> List:
        ...
        if self.use_dynamic_cache and self.dynamic_cache_manager:
            try:
                return self.dynamic_cache_manager.get_reliability_data(
    ...
    def get_reliability_summary(self) -> Dict:
        ...
        if self.use_dynamic_cache and self.dynamic_cache_manager:
            try:
                ...
                print(
                    f"⚠️ Warning: Could not get reliability summary from database: {e}"
                )
    ...
    def export_reliability_data(self, filepath: str) -> None:
        ...
        if self.use_dynamic_cache and self.dynamic_cache_manager:
            try:
                ...
                print(
                    f"✅ Reliability data exported via Dynamic Cache Manager to {filepath}"
                )
                return
            except Exception as e:
