...
from ..QCA_Prompts import ConfidenceScales, QCAPrompts
...
class InductiveCoder:
    ...
    def __init__(
        self, model_name: str, temperature: float, output_dir: str, config: dict = None
    ):
        ...
        api_key_env = CONFIG.get(
            "API_KEY_ENV", None
        )  # Benutzerdefinierter Env-Var-Name
        ...
        if api_key_env:
            print(f"   🔑 Custom API Key Env: {api_key_env}")
        ...
        analysis_mode = (
            config.get("ANALYSIS_MODE", "inductive")
            if config
            else CONFIG.get("ANALYSIS_MODE", "inductive")
        )
    ...
    def _create_proper_batches(
        self, segments: List[str], batch_size: int
    ) -> List[List[str]]:
        """
        ...
        print(
            f"🔀¦ Erstelle Batches: {len(segments)} Segmente -> Batch-Gröẞe {batch_size}"
        )
    ...
    async def _validate_and_integrate_strict(
        self,
        candidates: Dict[str, CategoryDefinition],
        ...
        for name, category in valid_candidates.items():
            ...
            if similar_existing:
                print(
                    f"ℹ️ '{name}' zu Ähnlich zu '{similar_existing}' - wird konsolidiert"
                )
                ...
                if consolidated:
                    ...
                    self.category_usage_history[similar_existing] = (
                        self.category_usage_history.get(similar_existing, 0) + 1
                    )
                continue
            ...
            if await self._meets_quality_standards(category):
                ...
                self.category_usage_history[name] = (
                    self.category_usage_history.get(name, 0) + 1
                )
                print(
                    f"✅ '{name}' validiert (Nutzung: {self.category_usage_history[name]})"
                )
            else:
    ...
    async def _consolidate_categories(
        ...
        for name1, name2, similarity in merge_candidates[:3]:  # Max 3 Merges pro Runde
            if name1 in consolidated and name2 in consolidated:
                print(
                    f"ðŸ”— Konsolidiere '{name1}' + '{name2}' (Ähnlichkeit: {similarity:.2f})"
                )
    ...
    async def _finalize_categories(
        self, categories: Dict[str, CategoryDefinition]
    ) -> Dict[str, CategoryDefinition]:
        """
        ...
        for name, category in categories.items():
            ...
            if usage_count >= min_usage:
                ...
                print(
                    f"⚠️ '{name}' entfernt (Zu wenig genutzt: {usage_count}, Mindest: {min_usage})"
                )
    ...
    def _update_usage_history(self, category_names: List[str]) -> None:
        ...
        print(
            f"    Aktuelle Nutzung: {dict(list(self.category_usage_history.items())[-3:])}"
        )
    ...
    def _format_existing_categories(
        self, categories: Dict[str, CategoryDefinition]
    ) -> str:
    ...
    async def develop_category_system(
        self,
        segments: List[str],
        ...
        for batch_idx, batch in enumerate(batches):
            ...
            # Analysiere Batch
            new_candidates = await self._analyze_batch_improved(
                batch, working_categories, analysis_mode
            )

            # Validiere und integriere neue Kategorien
            if new_candidates:
                validated_categories = await self._validate_and_integrate_strict(
                    new_candidates, working_categories
                )
            ...
            saturation_metrics = self._assess_comprehensive_saturation(
                working_categories, batch_idx + 1, len(batches)
            )

            print(f"\nℹ️ SÄTTIGUNGSANALYSE:")
            print(
                f"- Theoretische Sättigung: {saturation_metrics['theoretical_saturation']:.2f}"
            )
            ...
            if self._check_comprehensive_saturation(
                saturation_metrics, batch_idx + 1, len(batches)
            ):
                print(
            ...
            if (batch_idx + 1) % 3 == 0:
                ...
                working_categories = await self._consolidate_categories(
                    working_categories
                )
    ...
    def _assess_comprehensive_saturation(
        self,
    ...
    def _calculate_theoretical_saturation(
        self, categories: Dict[str, CategoryDefinition]
    ) -> float:
        """
    ...
    def _check_comprehensive_saturation(
        self,
        ...
        criteria_met = {
            ...
            "material_coverage": saturation_metrics["material_coverage"]
            >= min_material,
            "theoretical_saturation": saturation_metrics["theoretical_saturation"]
            >= 0.8,
    ...
    def _create_inductive_mode_prompt(
        self, segments_text: str, existing_categories: Dict[str, CategoryDefinition]
    ) -> str:
        """
    ...
    def _create_abductive_mode_prompt(
        self, segments_text: str, existing_categories: Dict[str, CategoryDefinition]
    ) -> str:
        """
    ...
    async def _analyze_batch_improved(
        self,
        batch: List[str],
        ...
        if analysis_mode == "inductive":
            prompt = self._create_inductive_mode_prompt(
                segments_text, existing_categories
            )
        elif analysis_mode == "abductive":
            prompt = self._create_abductive_mode_prompt(
                segments_text, existing_categories
            )
        else:
        ...
        try:
            ...
            response = await self.llm_provider.create_completion(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Du bist ein Experte fuer qualitative Inhaltsanalyse. Antworte auf deutsch. Antworte ausschliesslich mit einem JSON-Objekt.",
                    },
            ...
            for cat_data in result.get("new_categories", []):
                ...
                if confidence >= self.MIN_CONFIDENCE:
                    candidates[cat_data["name"]] = self._create_category_definition(
                        cat_data
                    )
                    print(
                        f"✅ Neuer Kandidat: '{cat_data['name']}' (Konfidenz: {confidence:.2f})"
                    )
    ...
    def _log_saturation_progress(self, saturation_metrics: Dict[str, float]) -> None:
        ...
        print(
            f"   🎯 Theoretische Sättigung: {saturation_metrics['theoretical_saturation']:.1%}"
        )
        print(f"   ℹ️ Materialabdeckung: {saturation_metrics['material_coverage']:.1%}")
        print(
            f"   ℹ️ StabilitÄt: {saturation_metrics['stable_batches']} Batches ohne neue Kategorien"
        )
    ...
    def _show_development_summary(
        self,
        final_categories: Dict[str, CategoryDefinition],
        initial_categories: Dict[str, CategoryDefinition],
    ) -> None:
        """
    ...
    async def _enhance_category_definition(
        self, category: CategoryDefinition
        ...
        try:
            ...
            response = await self.llm_provider.create_completion(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Du bist ein Experte fuer qualitative Inhaltsanalyse.",
                    },
                    {"role": "user", "content": prompt},
                ],
                temperature=self.temperature,
    ...
    async def analyze_grounded_batch(
        self, segments: List[str], material_percentage: float
    ) -> Dict[str, Any]:
        """
        ...
        try:
            # Cache-Key erstellen
            cache_key = (tuple(segments), "grounded")
            ...
            response = await self.llm_provider.create_completion(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Du bist ein Experte fuer qualitative Inhaltsanalyse. Du antwortest auf deutsch.",
                    },
                    {"role": "user", "content": prompt},
                ],
                temperature=self.temperature,
            ...
            if (
                abstraction_quality
                and "keyword_subcode_distinction" in abstraction_quality
            ):
            ...
            if hasattr(self, "collected_subcodes"):
                ...
                print(
                    f"Bisher gesammelt: {total_collected} Subcodes mit ihren Keywords"
                )
    ...
    def _create_batches(
        self, segments: List[str], batch_size: int = None
    ) -> List[List[str]]:
        """
    ...
    async def _generate_main_categories_from_subcodes(
        self, initial_categories: Dict[str, CategoryDefinition] = None
    ) -> Dict[str, CategoryDefinition]:
        """
        ...
        try:
            ...
            if hasattr(self, "collected_subcodes") and self.collected_subcodes:
                ...
                print(
                    f"🧾 Verwende Subcodes aus InductiveCoder: {len(collected_subcodes)}"
                )
            elif hasattr(self, "analysis_manager") and hasattr(
                self.analysis_manager, "collected_subcodes"
            ):
                collected_subcodes = self.analysis_manager.collected_subcodes
                print(
                    f"🧾 Verwende Subcodes aus AnalysisManager: {len(collected_subcodes)}"
                )
            else:
                print(
                    "❌ Keine gesammelten Subcodes gefunden - prÜfe verfügbare Attribute:"
                )
            ...
            if len(collected_subcodes) < 5:
                print(
                    f"❌ Zu wenige Subcodes fuer Hauptkategorien-Generierung: {len(collected_subcodes)} < 5"
                )
                return initial_categories or {}
            ...
            avg_confidence = sum(
                s.get("confidence", 0) for s in collected_subcodes
            ) / len(collected_subcodes)
            ...
            print(
                f"   - Top Keywords: {', '.join([f'{kw}({count})' for kw, count in top_keywords[:8]])}"
            )
            ...
            response = await self.llm_provider.create_completion(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Du bist ein Experte fuer Grounded Theory und qualitative Inhaltsanalyse. Du antwortest auf deutsch.",
                    },
            ...
            for i, category_data in enumerate(result.get("main_categories", []), 1):
                ...
                if name and definition:
                    ...
                    # Zeige Details
                    characteristic_keywords = ", ".join(
                        category_data.get("characteristic_keywords", [])[:5]
                    )
                    print(
                        f"   {i}. 🔀 '{name}': {len(subcategories)} Subcodes zugeordnet"
                    )
                    print(f"      Keywords: {characteristic_keywords}")
                    print(
                        f"      Subcodes: {', '.join(assigned_subcodes[:3])}{'...' if len(assigned_subcodes) > 3 else ''}"
                    )
            ...
            if meta:
                ...
                print(
                    f"   - Verarbeitete Subcodes: {meta.get('total_subcodes_processed', len(subcodes_data))}"
                )
                print(f"   - Generierte Hauptkategorien: {len(grounded_categories)}")
                print(
                    f"   - Theoretische Sättigung: {meta.get('theoretical_saturation', 0):.2f}"
                )
                print(f"   - Subcode-Abdeckung: {meta.get('coverage', 0):.2f}")

            # Prüfe Subcode-Zuordnung
            mapped_subcodes = (
                set(subcode_mapping.values()) if subcode_mapping else set()
            )
    ...
    def _find_similar_category(
        ...
        for existing_name, existing_cat in existing_categories.items():
            ...
            if similarity > self.similarity_threshold:
                print(
                    f"🕵️ Ähnliche Kategorie gefunden: '{category.name}' â†” '{existing_name}' ({similarity:.2f})"
                )
    ...
    def _document_reliability_results(
        ...
        try:
            ...
            for category, frequency in sorted(
                category_frequencies.items(), key=lambda x: x[1], reverse=True
            ):
            ...
            report.extend(
                [
                    ...
                    + max(category_frequencies.items(), key=lambda x: x[1])[0],
                    "- Least frequently used category: "
                    + min(category_frequencies.items(), key=lambda x: x[1])[0],
                    "- Number of categories with single use: "
                    + str(sum(1 for x in category_frequencies.values() if x == 1)),
                    "\n### Coder Performance",
                    "- Average segments per coder: "
                    + f"{total_segments / total_coders:.1f}"
                    if total_coders > 0
                    else "N/A",
                ]
            )
    ...
    async def _meets_quality_standards(self, category: CategoryDefinition) -> bool:
        ...
        if len(category.definition.split()) < 5:  # reduziert von 10
            print(
                f"⚠️ '{category.name}': Definition zu kurz ({len(category.definition.split())} Wörter)"
            )
            return False
        ...
        if not category.examples or len(category.examples) < 1:  # reduziert von 2
            print(
                f"⚠️ '{category.name}': Zu wenige Beispiele ({len(category.examples) if category.examples else 0})"
            )
            return False
    ...
    async def _auto_merge_categories(
        self, cat1: CategoryDefinition, cat2: CategoryDefinition, name1: str, name2: str
    ) -> Optional[CategoryDefinition]:
        """
        ...
        try:
            ...
            combined_definition = await self._merge_definitions_intelligent(
                cat1.definition, cat2.definition
            )
    ...
    async def _merge_definitions_intelligent(self, def1: str, def2: str) -> str:
        ...
        prompt = self.prompt_handler.get_definition_enhancement_prompt(
            {"definition1": def1, "definition2": def2}
        )
    ...
    def _calculate_category_similarity(
        self, cat1: CategoryDefinition, cat2: CategoryDefinition
    ) -> float:
        """
        ...
        name_similarity = (
            self._calculate_text_similarity(cat1.name.lower(), cat2.name.lower()) * 0.3
        )

        # 2. Definition-Ähnlichkeit (50%)
        def_similarity = (
            self._calculate_text_similarity(cat1.definition, cat2.definition) * 0.5
        )
    ...
    async def _merge_categories_intelligent(
        self, cat1: CategoryDefinition, cat2: CategoryDefinition, name1: str, name2: str
    ) -> Optional[CategoryDefinition]:
        """
    ...
    def _log_performance(
        self, num_segments: int, num_categories: int, processing_time: float
    ) -> None:
        """
