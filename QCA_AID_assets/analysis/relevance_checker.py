...
class RelevanceChecker:
    ...
    def __init__(self, model_name: str, batch_size: int = 5, temperature: float = 0.3):
        ...
        print(f"- {len(self.exclusion_rules)} Ausschlussregeln geladen")
        print(
            f"- Mehrfachkodierung: {'Aktiviert' if self.multiple_codings_enabled else 'Deaktiviert'}"
        )
    ...
    async def check_multiple_category_relevance(
        self, segments: List[Tuple[str, str]], categories: Dict[str, CategoryDefinition]
    ) -> Dict[str, List[Dict]]:
        """
        ...
        try:
            print(
                f"🚀 Parallele Mehrfachkodierungs-Pruefung: {len(uncached_segments)} Segmente"
            )
            ...
            for cat_name, cat_def in categories.items():
                if cat_name not in ["Nicht kodiert", "Kein Kodierkonsens"]:
                    examples_list = (
                        list(cat_def.examples)
                        if isinstance(cat_def.examples, set)
                        else cat_def.examples
                    )
                    category_descriptions.append(
                        {
                            "name": cat_name,
                            "definition": cat_def.definition[:200] + "..."
                            if len(cat_def.definition) > 200
                            else cat_def.definition,
                            "examples": examples_list[:2] if examples_list else [],
                        }
                    )

            # 🚀 PARALLEL: Hilfsfunktion für einzelnes Segment
            async def check_single_segment_multiple(
                ...
                try:
                    ...
                    try:
                        ...
                        print(
                            f"‼️ JSONDecodeError in check_multiple_category_relevance for segment {segment_id}: {e}"
                        )
                    ...
                    print(
                        f"[WARN] Fehler bei Mehrfachkodierungs-Pruefung {segment_id}: {str(e)}"
                    )
            ...
            print(
                f"⚡ {successful_checks} Mehrfachkodierungs-Prüfungen in {processing_time:.2f}s"
            )
            print(
                f"🔄 {segments_with_multiple} Segmente mit Mehrfachkodierung identifiziert"
            )
            ...
            for segment_id, text in segments:
                if segment_id in multiple_coding_results:
                    ...
                    final_results[segment_id] = self.multiple_coding_cache.get(
                        segment_id, []
                    )
    ...
    async def check_relevance_batch(
        ...
        # Filtere bereits gecachte Segmente
        uncached_segments = [
            (sid, text) for sid, text in segments if sid not in self.relevance_cache
        ]
        ...
        try:
            ...
            if len(uncached_segments) <= 5:
                ...
                print(
                    f"   📦 Verwende normale Batch-Methode für {len(uncached_segments)} Segmente"
                )
                ...
                prompt = self.prompt_handler.get_relevance_check_prompt(
                    segments_text=segments_text, exclusion_rules=self.exclusion_rules
                )
                ...
                response = await self.llm_provider.create_completion(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "Du bist ein Experte für qualitative Inhaltsanalyse. Du antwortest auf deutsch.",
                        },
                        {"role": "user", "content": prompt},
                    ],
                    temperature=self.temperature,
                ...
                if len(segment_results) != len(uncached_segments):
                    print(
                        f"⚠️ Warnung: Anzahl Ergebnisse ({len(segment_results)}) != Anzahl Segmente ({len(uncached_segments)})"
                    )
                    ...
                    for i, (segment_id, _) in enumerate(uncached_segments):
                        ...
                        is_relevant = segment_result.get(
                            "is_relevant", True
                        )  # Default: relevant
                        ...
                        self.relevance_details[segment_id] = {
                            ...
                            "relevance_strength": segment_result.get(
                                "relevance_strength",
                                segment_result.get("confidence", 0.8),
                            ),
                            "classification_confidence": segment_result.get(
                                "classification_confidence",
                                segment_result.get("confidence", 0.8),
                            ),
                            "key_aspects": segment_result.get("key_aspects", []),
                            "aspects_found": segment_result.get(
                                "aspects_found",
                                segment_result.get("core_topics_found", []),
                            ),
                            "reasoning": segment_result.get(
                                "reasoning",
                                segment_result.get(
                            ...
                            "exclusion_match": segment_result.get(
                ...
                print(
                    f"   🚀 Verwende Parallelisierung in Sub-Batches fuer {len(uncached_segments)} Segmente"
                )
                ...
                async def process_sub_batch(
                    ...
                    try:
                        ...
                        try:
                            ...
                            print(f"‼️ Raw LLM response: {llm_response.content}")
                            print(
                                f"⚠️ Fallback für Sub-Batch: Markiere alle als relevant"
                            )
                        ...
                        if len(segment_results) != len(sub_batch):
                            ...
                            for i, (segment_id, _) in enumerate(sub_batch):
                                ...
                                self.relevance_details[segment_id] = {
                                    ...
                                    "key_aspects": segment_result.get(
                                        "key_aspects", []
                                    ),
                                    "aspects_found": segment_result.get(
                                        "aspects_found",
                                    ...
                                    "is_relevant": is_relevant,
                                    "main_themes": segment_result.get(
                                        "main_themes", []
                                    ),
                                    "exclusion_match": segment_result.get(
                                        "exclusion_match", False
                ...
                print(
                    f"   ⚡ {successful_batches} Sub-Batches in {processing_time:.2f}s verarbeitet"
                )

            # Debug-Ausgabe der Ergebnisse
            relevant_count = sum(
                1 for is_relevant in relevance_results.values() if is_relevant
            )
            print(
                f"   ℹ️ Relevanz-Ergebnisse: {relevant_count}/{len(relevance_results)} als relevant eingestuft"
            )
            ...
            for segment_id, text in segments:
                if segment_id in relevance_results:
                    ...
                    final_results[segment_id] = self.relevance_cache.get(
                        segment_id, True
                    )
    ...
    async def check_relevance_with_category_preselection(
        self,
        ...
        print(
            f"🧫 Erweiterte Relevanzprüfung mit Kategorie-Vorauswahl fuer {len(segments)} Segmente..."
        )
        ...
        try:
            ...
            for i, (segment_id, text) in enumerate(segments_to_process):
                if i < len(segment_results):
                    ...
                    result = {
                        ...
                        "preferred_categories": segment_result.get(
                            "preferred_categories", []
                        ),
            ...
            print(
                f"✅ Erweiterte Relevanzprüfung abgeschlossen: {len([r for r in results.values() if r['is_relevant']])} relevante Segmente"
            )
