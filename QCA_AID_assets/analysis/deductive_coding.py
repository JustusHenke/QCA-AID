...
from ..core.config import CONFIG, DEDUKTIVE_KATEGORIEN, FORSCHUNGSFRAGE, KODIERREGELN
        ...
        try:
            ...
            if "category_def" in locals():
                ...
                if isinstance(category_def, CategoryDefinition):
                    ...
                    print(
                        "Kategorie-Daten:",
                        json.dumps(category_def, indent=2, ensure_ascii=False),
                    )
            raise
...
class DeductiveCoder:
    ...
    def __init__(
        ...
        if analysis_mode == "grounded":
            ...
            print(
                f"🧾 Kodierer {coder_id}: Grounded Mode - startet ohne deduktive Kategorien"
            )
        else:
            ...
            try:
                ...
                print(
                    f"🧾 Kodierer {coder_id}: {len(self.current_categories)} deduktive Kategorien geladen ({analysis_mode} mode)"
                )
            except Exception as e:
                print(
                    f"❌ Fehler beim Laden der Kategorien fuer Kodierer {coder_id}: {str(e)}"
                )
                self.current_categories = {}
        ...
        try:
            ...
            print(
                f"🤖 LLM Provider '{provider_name}' fuer Kodierer {coder_id} initialisiert"
            )
            if base_url:
        ...
        if analysis_mode == "grounded":
            ...
            print(
                f"   ℹ️ Grounded Mode: Kodierer {coder_id} ohne vordefinierte Kategorien initialisiert"
            )
        else:
    ...
    async def update_category_system(
        ...
        try:
            ...
            if analysis_mode == "grounded":
                ...
                print(
                    f"   ℹ️ Grounded Mode: Kodierer {self.coder_id} aktualisiert mit {len(grounded_categories)} rein induktiven Kategorien"
                )
                print(
                    f"   🔀 Ausgeschlossen: {len(categories) - len(grounded_categories)} deduktive Kategorien"
                )
                ...
                print(
                    f"   ℹ️ {analysis_mode.upper()} Mode: Kodierer {self.coder_id} aktualisiert mit {len(categories_to_use)} Kategorien"
                )
            ...
            categories_dict = {
                name: {
                    ...
                    if isinstance(cat.examples, set)
                    else cat.examples,
                    "rules": list(cat.rules)
                    if isinstance(cat.rules, set)
                    else cat.rules,
                    "subcategories": dict(cat.subcategories)
                    if isinstance(cat.subcategories, set)
                    else cat.subcategories,
                }
            ...
            response = await self.llm_provider.create_completion(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Du bist ein QCA-Kodierer. Antworte auf deutsch.",
                    },
            ...
            if "verstanden" in llm_response.content.lower():
                ...
                print(
                    f"❌ Unerwartete Antwort von Kodierer {self.coder_id}: {llm_response.content}"
                )
    ...
    async def code_chunk(
        self,
        chunk: str,
        categories: Optional[Dict[str, CategoryDefinition]] = None,
        ...
        try:
            ...
            if not current_categories:
                print(
                    f"Fehler: Kein Kategoriensystem fuer Kodierer {self.coder_id} verfügbar"
                )
                return None
            ...
            if preferred_cats:
                ...
                if filtered_categories:
                    print(
                        f"    🎯 Gefilterte Kodierung fuer {self.coder_id}: {len(filtered_categories)}/{len(current_categories)} Kategorien"
                    )
                    print(f"    🔀 Fokus auf: {', '.join(preferred_cats)}")
                    effective_categories = filtered_categories
                else:
                    print(
                        f"    ❌ Keine der bevorzugten Kategorien {preferred_cats} gefunden - nutze alle Kategorien"
                    )
            ...
            for (
                name,
                cat,
            ) in effective_categories.items():  # FIX: Nutze gefilterte Kategorien
                category_info = {
                    "name": name,
                    "definition": cat.definition,
                    "examples": list(cat.examples)
            ...
            try:
                ...
                response = await self.llm_provider.create_completion(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "Du bist ein Experte fuer qualitative Inhaltsanalyse. Du antwortest auf deutsch. Antworte ausschliesslich mit einem JSON-Objekt.",
                        },
                        {"role": "user", "content": prompt},
                    ],
                    temperature=self.temperature,
                ...
                try:
                    ...
                    print(
                        f"‼️[{self.coder_id}] Raw LLM response: {llm_response.content}"
                    )
                ...
                if result and isinstance(result, dict):
                    ...
                    validated_subcats = (
                        CategoryValidator.validate_subcategories_for_category(
                            original_subcats,
                            main_category,
                            self.current_categories,
                            warn_only=False,
                    ...
                    coding_result = CodingResult(
                        ...
                        text_references=tuple(
                            result.get("text_references", [])
                        ),  # ✅ Tuple
                        uncertainties=tuple(result.get("uncertainties", []))
            ...
            print(
                f"Allgemeiner Fehler bei der Kodierung durch {self.coder_id}: {str(e)}"
            )
            return None

    async def code_chunk_with_focus(
        self,
        chunk: str,
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    print(
                        f"‼️[{self.coder_id}] JSONDecodeError in code_chunk_with_progressive_context: {e}"
                    )
                    print(
                        f"‼️[{self.coder_id}] Raw LLM response: {llm_response.content}"
                    )
                ...
                if result and isinstance(result, dict):
                    if result.get("category"):
                        ...
                        print(
                            f"      {focus_icon} Fokuskodierung von {self.coder_id}: 📝  {result.get('category', '')}"
                        )
                        print(
                            f"      ✅ Subkategorien: 📝  {', '.join(result.get('subcategories', []))}"
                        )
                        print(f"      ✅ Keywords: 📝  {result.get('keywords', '')}")

                        if not followed_focus:
                            deviation_reason = focus_adherence.get(
                                "deviation_reason", "Nicht angegeben"
                            )
                        ...
                        if focus_adherence:
                            ...
                            chosen_score = focus_adherence.get(
                                "chosen_category_score", 0
                            )
                            print(
                                f"      🧾 Fokus-Score: {focus_score:.2f}, GewÄhlt-Score: {chosen_score:.2f}"
                            )
                        ...
                        return CodingResult(
                            ...
                            confidence=result.get(
                                "confidence",
                                {"total": 0.0, "category": 0.0, "subcategories": 0.0},
                            ),
            ...
            print(
                f"Fehler bei der fokussierten Kodierung durch {self.coder_id}: {str(e)}"
            )
            return None
    ...
    async def _check_relevance(self, chunk: str) -> bool:
        ...
        try:
            ...
            if result.get("is_relevant"):
                print(
                    f"✅ Relevanz bestÄtigt (Konfidenz: {result.get('confidence', 0):.2f})"
                )
                ...
                print(
                    f"⚠️ Nicht relevant: {result.get('justification', 'Keine BegrÜndung')}"
                )
    ...
    def _validate_coding(self, result: dict) -> Optional[CodingResult]:
        ...
        try:
            return CodingResult(
                ...
                justification=result.get("justification", ""),
                confidence=result.get(
                    "confidence", {"total": 0, "category": 0, "subcategories": 0}
                ),
