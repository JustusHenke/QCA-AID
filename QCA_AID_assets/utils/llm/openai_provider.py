...
class OpenAIProvider(LLMProvider):
    ...
    def __init__(
        self, base_url: Optional[str] = None, api_key_env: Optional[str] = None
    ) -> None:
        """
        Initialize OpenAIProvider with optional custom base URL and API key env var name.

        Args:
            base_url: Custom base URL for OpenAI-compatible endpoints (e.g., GWDG)
            api_key_env: Name der Umgebungsvariable für den API-Key
                         (z.B. 'GWDG_API_KEY', 'CUSTOM_API_KEY').
    ...
    def initialize_client(self) -> None:
        ...
        Wenn self.api_key_env gesetzt ist, wird dieser Env-Var-Name verwendet
        anstatt OPENAI_API_KEY (für Custom API Endpoints).
        ...
        try:
            ...
            # Benutzerdefinierter Env-Var-Name oder Standard
            env_var = getattr(self, "api_key_env", None) or "OPENAI_API_KEY"
            api_key = os.getenv(env_var)
            if not api_key:
                raise ValueError(f"{env_var} nicht in Umgebungsvariablen gefunden")

            # Initialize with custom base_url if provided
            if self.base_url:
                self.client = AsyncOpenAI(api_key=api_key, base_url=self.base_url)
                print(
                    f"✅ OpenAI Client erfolgreich initialisiert (Custom Base URL: {self.base_url}, Env-Var: {env_var})"
                )
            else:
                self.client = AsyncOpenAI(api_key=api_key)
                print(
                    f"✅ OpenAI Client erfolgreich initialisiert (Env-Var: {env_var})"
                )

        except ImportError:
            raise ImportError(
                "OpenAI Bibliothek nicht installiert. Bitte installieren Sie: pip install openai"
            )
        ...
        try:
            ...
            try:
                ...
                if "temperature" in error_msg.lower() and "temperature" in params:
                    ...
                    print(
                        f"⚠️  Model {model} unterstützt temperature-Parameter NICHT. Retry ohne..."
                    )

                    # Versuche erneut ohne temperature
                    params_without_temp = {
                        k: v for k, v in params.items() if k != "temperature"
                    }
                    try:
                        ...
                        if "response_format" in retry_error_msg.lower():
                            print(
                                f"⚠️  Model {model} unterstützt auch response_format nicht. Retry ohne beide..."
                            )
                            params_minimal = {
                                k: v
                                for k, v in params.items()
                                if k not in ["temperature", "response_format"]
                            }
                            try:
                                response = await self.client.chat.completions.create(
                                    **params_minimal
                                ...
                                print(
                                    f"❌ ‼️ Auch minimal-Versuch fehlgeschlagen: {str(minimal_error)[:200]}"
                                )
                            ...
                            # Anderer Fehler
                            print(
                                f"❌ ‼️ Retry ohne temperature fehlgeschlagen: {retry_error_msg[:200]}"
                            )
                            token_counter.track_error(self.model_name)
                            raise
                else:
                    ...
                    print(
                        f"❌ ‼️ API-Fehler (nicht temperature-bezogen): {error_msg[:200]}"
                    )
                    token_counter.track_error(self.model_name)
                    raise
