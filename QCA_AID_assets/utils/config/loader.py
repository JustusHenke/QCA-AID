...
class ConfigLoader:
    ...
    def __init__(self, script_dir: str, global_config: Dict) -> None:
        ...
        # FIX: Prüfe .qca-aid-project.json für zuletzt genutzten Codebook-Pfad
        self._resolve_last_codebook_path()

    def _resolve_last_codebook_path(self) -> None:
        """
        Liest den Codebook-Pfad aus (Reihenfolge):
        1. Env-Var QCA_AID_CODEBOOK_PATH (vom Webapp-Subprocess übergeben)
        2. .qca-aid-project.json → last_codebook_file
        3. Fallback: Standard-Pfad (QCA-AID-Codebook.json im script_dir)
        """
        import json as _json
        from pathlib import Path

        # 1. Höchste Priorität: Env-Var vom AnalysisRunner
        env_codebook = os.environ.get("QCA_AID_CODEBOOK_PATH")
        if env_codebook:
            candidate = Path(env_codebook).expanduser().resolve()
            if candidate.exists():
                self.json_path = str(candidate)
                xlsx_candidate = candidate.with_suffix(".xlsx")
                if xlsx_candidate.exists():
                    self.excel_path = str(xlsx_candidate)
                print(f"   📂 Codebook aus Env-Var: {candidate}")
                return
            else:
                print(f"   ⚠️ Codebook aus Env-Var nicht gefunden: {candidate}")

        # 2. .qca-aid-project.json → last_codebook_file
        project_settings_candidates = [
            Path(self.script_dir) / ".qca-aid-project.json",
            Path(self.script_dir).parent / ".qca-aid-project.json",
        ]

        for settings_path in project_settings_candidates:
            if settings_path.exists():
                try:
                    with open(settings_path, "r", encoding="utf-8") as f:
                        data = _json.load(f)

                    last_codebook = data.get("last_codebook_file")
                    if last_codebook:
                        candidate = Path(last_codebook).expanduser().resolve()
                        if candidate.exists():
                            self.json_path = str(candidate)
                            # Korrespondierende XLSX im selben Verzeichnis
                            xlsx_candidate = candidate.with_suffix(".xlsx")
                            if xlsx_candidate.exists():
                                self.excel_path = str(xlsx_candidate)
                            print(f"   📂 Letztes Codebook: {candidate}")
                            return
                        else:
                            print(
                                f"   ⚠️ Gespeichertes Codebook nicht gefunden: {candidate}"
                            )
                except Exception as e:
                    print(f"   ⚠️ Konnte .qca-aid-project.json nicht lesen: {e}")
                break  # Nur erste gefundene Settings-Datei verwenden
    ...
    def load_codebook(self) -> bool:
        ...
        """
        # Zeige使用的Config-Datei an
        print(f"\n📋 Konfigurationsdatei: {self.json_path}")
            ...
            if success:
                ...
                if not xlsx_exists:
                    try:
                        ...
                        print(
                            f"⚠️ Warnung: Konnte Excel-Datei nicht erstellen: {str(e)}"
                        )
        ...
        try:
            ...
            print(
                f"Excel-Datei erfolgreich geladen. verfügbare Sheets: {wb.sheetnames}"
            )
            ...
            if kategorien:
                ...
                if not json_exists:
                    try:
                        ...
                        json_data = ConfigConverter.qca_aid_xlsx_to_json(
                            self.excel_path
                        )
                        ConfigConverter.save_json(json_data, self.json_path)
                ...
                if kategorien:
                    ...
                    if not json_exists:
                        try:
                            ...
                            print(
                                f"⚠️ Warnung: Konnte JSON-Datei nicht erstellen: {str(e)}"
                            )
    ...
    def _load_from_json(self, json_path: str) -> bool:
        ...
        try:
            ...
            if "forschungsfrage" in json_data:
                ...
                print(
                    f"Forschungsfrage geladen: {json_data['forschungsfrage'][:50]}..."
                )
            ...
            if "kodierregeln" in json_data:
                ...
                print(
                    f"  - Allgemeine Regeln: {len(json_data['kodierregeln'].get('general', []))}"
                )
                print(
                    f"  - Formatregeln: {len(json_data['kodierregeln'].get('format', []))}"
                )
                print(
                    f"  - Ausschlussregeln: {len(json_data['kodierregeln'].get('exclusion', []))}"
                )
            ...
            if "config" in json_data:
                ...
                if "MANUAL_CODING_ENABLED" in config_normalized:
                    self.global_config["MANUAL_CODING_ENABLED"] = bool(
                        config_normalized["MANUAL_CODING_ENABLED"]
                    )
                    print(
                        f"[CONFIG] MANUAL_CODING_ENABLED geladen: {self.global_config['MANUAL_CODING_ENABLED']}"
                    )
    ...
    def _load_deduktive_kategorien(self, wb) -> Dict:
        ...
        try:
            ...
            sub_sub_key_idx = (
                headers.index("Sub-Sub-Key") if "Sub-Sub-Key" in headers else None
            )
            ...
            for row_idx, row in enumerate(sheet.iter_rows(min_row=2), 2):
                try:
                    ...
                    sub_sub_key = (
                        row[sub_sub_key_idx].value
                        if sub_sub_key_idx is not None and row[sub_sub_key_idx].value
                        else None
                    )
                    ...
                    if current_category and sub_key:
                        ...
                        if sub_key == "definition":
                            ...
                            kategorien[current_category]["subcategories"][
                                sub_sub_key
                            ] = value
    ...
    def _load_config(self, wb) -> bool:
        ...
        for _, row in df.iterrows():
                ...
                if sub_key.startswith("["):  # Für Listen wie CODER_SETTINGS
                    ...
                    if pd.isna(sub_sub_key):
                        ...
                        if sub_key in [
                            ...
                            if isinstance(value, str):
                                config[key][sub_key] = value.lower() in [
                                    "true",
                                    "1",
                                    "yes",
                                    "ja",
                            ...
                            print(
                                f"[CONFIG] {sub_key} geladen: {config[key][sub_key]} (ursprünglich: '{value}')"
                            )
                        elif (
                            sub_key
                            in [
                            ...
                            try:
                                ...
                                print(
                                    f"[CONFIG] {sub_key} geladen: {config[key][sub_key]}"
                                )
                                ...
                                print(
                                    f"[CONFIG] Ungültiger {sub_key} Wert '{value}', verwende Standard: {config[key][sub_key]}"
                                )
                        elif sub_key in [
                            "MULTIPLE_CODING_THRESHOLD",
                            "SIMILARITY_THRESHOLD",
            ...
            if key == "EXPORT_ANNOTATED_PDFS" and pd.isna(sub_key):
                if isinstance(value, str):
                    config["EXPORT_ANNOTATED_PDFS"] = value.lower() in [
                        "true",
                        "1",
                        "yes",
                        "ja",
                ...
                print(
                    f"[CONFIG] EXPORT_ANNOTATED_PDFS geladen: {config['EXPORT_ANNOTATED_PDFS']}"
                )
        ...
        if "ANALYSIS_MODE" in config:
            ...
            if config["ANALYSIS_MODE"] not in valid_modes:
                print(
                    f"[CONFIG] Warnung: Ungültiger ANALYSIS_MODE '{config['ANALYSIS_MODE']}'. Verwende 'deductive'."
                )
            ...
            config["ANALYSIS_MODE"] = "deductive"
            print(
                f"[CONFIG] ANALYSIS_MODE nicht gefunden, verwende Standard: deductive"
            )
        ...
        if "REVIEW_MODE" in config:
            ...
            if config["REVIEW_MODE"] not in valid_modes:
                print(
                    f"[CONFIG] Warnung: Ungültiger REVIEW_MODE '{config['REVIEW_MODE']}'. Verwende 'consensus'."
                )
        ...
        if "ATTRIBUTE_LABELS" not in config:
            config["ATTRIBUTE_LABELS"] = {
                "attribut1": "Attribut1",
                "attribut2": "Attribut2",
                "attribut3": "Attribut3",
            }
        elif (
            isinstance(config.get("ATTRIBUTE_LABELS"), dict)
            and "attribut3" not in config["ATTRIBUTE_LABELS"]
        ):
            ...
            if key in config and isinstance(config[key], dict):
                for param_name in [
                    ...
                    if param_name in config[key]:
                        ...
                        print(
                            f"[CONFIG] Extrahierte {param_name} aus {key}: {config[param_name]}"
                        )
        ...
        try:
                ...
                if key in loaded_config:
                    ...
                    if os.path.isabs(path_value):
                        ...
                        resolved_path = os.path.join(
                            project_root, path_value.lstrip("/\\")
                        )
            ...
            if "CHUNK_SIZE" in loaded_config:
                try:
                    ...
                    if chunk_size < 1:
                        print(
                            f"⚠️ Warnung: CHUNK_SIZE muss >= 1 sein. Wert '{chunk_size}' ist ungültig."
                        )
                        print(
                            f"   Verwende Standardwert: {DEFAULT_VALUES['CHUNK_SIZE']}"
                        )
                    ...
                    print(
                        f"⚠️ Warnung: Ungültiger CHUNK_SIZE Wert '{loaded_config['CHUNK_SIZE']}'."
                    )
            ...
            if "CHUNK_OVERLAP" in loaded_config:
                try:
                    ...
                    if chunk_overlap < 0:
                        print(
                            f"⚠️ Warnung: CHUNK_OVERLAP muss >= 0 sein. Wert '{chunk_overlap}' ist ungültig."
                        )
                        print(
                            f"   Verwende Standardwert: {DEFAULT_VALUES['CHUNK_OVERLAP']}"
                        )
                        self.global_config["CHUNK_OVERLAP"] = DEFAULT_VALUES[
                            "CHUNK_OVERLAP"
                        ]
                    else:
                    ...
                    print(
                        f"⚠️ Warnung: Ungültiger CHUNK_OVERLAP Wert '{loaded_config['CHUNK_OVERLAP']}'."
                    )
                    print(
                        f"   Verwende Standardwert: {DEFAULT_VALUES['CHUNK_OVERLAP']}"
                    )
            ...
            if self.global_config["CHUNK_OVERLAP"] >= self.global_config["CHUNK_SIZE"]:
                print(
                    f"⚠️ Warnung: CHUNK_OVERLAP ({self.global_config['CHUNK_OVERLAP']}) >= CHUNK_SIZE ({self.global_config['CHUNK_SIZE']})."
                )
            ...
            if "BATCH_SIZE" in loaded_config:
                try:
                    ...
                    if batch_size < 1:
                        print(
                            f"⚠️ Warnung: BATCH_SIZE muss >= 1 sein. Wert '{batch_size}' ist ungültig."
                        )
                        print(
                            f"   Verwende Standardwert: {DEFAULT_VALUES['BATCH_SIZE']}"
                        )
                        self.global_config["BATCH_SIZE"] = DEFAULT_VALUES["BATCH_SIZE"]
                    elif batch_size > 20:
                        print(
                            f"⚠️ Warnung: BATCH_SIZE > 20 könnte Performance-Probleme verursachen. Wert '{batch_size}' ist zu hoch."
                        )
                        print(
                            f"   Verwende Standardwert: {DEFAULT_VALUES['BATCH_SIZE']}"
                        )
                    ...
                    print(
                        f"⚠️ Warnung: Ungültiger BATCH_SIZE Wert '{loaded_config['BATCH_SIZE']}'."
                    )
            ...
            float_thresholds = {
                "MULTIPLE_CODING_THRESHOLD": DEFAULT_VALUES[
                    "MULTIPLE_CODING_THRESHOLD"
                ],
                "SIMILARITY_THRESHOLD": DEFAULT_VALUES["SIMILARITY_THRESHOLD"],
                "PDF_ANNOTATION_FUZZY_THRESHOLD": DEFAULT_VALUES[
                    "PDF_ANNOTATION_FUZZY_THRESHOLD"
                ],
            }
                ...
                if threshold_name in loaded_config:
                    try:
                        ...
                        if threshold < 0.0 or threshold > 1.0:
                            print(
                                f"⚠️ Warnung: {threshold_name} muss zwischen 0.0 und 1.0 liegen. Wert '{threshold}' ist ungültig."
                            )
                        ...
                        print(
                            f"⚠️ Warnung: Ungültiger {threshold_name} Wert '{loaded_config[threshold_name]}'."
                        )
                        print(f"   Verwende Standardwert: {default_value}")
                        self.global_config[threshold_name] = default_value
                else:
            ...
            if "ANALYSIS_MODE" in loaded_config:
                ...
                if analysis_mode not in valid_analysis_modes:
                    ...
                    print(
                        f"   Verwende Standardwert: '{DEFAULT_VALUES['ANALYSIS_MODE']}'"
                    )
                    self.global_config["ANALYSIS_MODE"] = DEFAULT_VALUES[
                        "ANALYSIS_MODE"
                    ]
                else:
            ...
            if "REVIEW_MODE" in loaded_config:
                ...
                if review_mode not in valid_review_modes:
                    ...
                    print(
                        f"   Verwende Standardwert: '{DEFAULT_VALUES['REVIEW_MODE']}'"
                    )
            ...
            if "CODER_SETTINGS" in loaded_config:
                ...
                print(
                    f"🩹 CODER_SETTINGS geladen: {len(self.global_config['CODER_SETTINGS'])} Coder"
                )
                for i, coder in enumerate(self.global_config["CODER_SETTINGS"]):
                    print(
                        f"   Coder {i}: ID='{coder['coder_id']}', Temp={coder['temperature']}"
                    )
            ...
            if "CODE_WITH_CONTEXT" in loaded_config:
                ...
                if isinstance(value, str):
                    self.global_config["CODE_WITH_CONTEXT"] = value.lower() in (
                        "true",
                        "ja",
                        "yes",
                        "1",
                ...
                print(
                    f"🩹 CODE_WITH_CONTEXT = {self.global_config['CODE_WITH_CONTEXT']}"
                )
            else:
            ...
            if "MULTIPLE_CODINGS" in loaded_config:
                ...
                if isinstance(value, str):
                    self.global_config["MULTIPLE_CODINGS"] = value.lower() in (
                        "true",
                        "ja",
                        "yes",
                        "1",
            ...
            if "MANUAL_CODING_ENABLED" in loaded_config:
                ...
                if isinstance(value, str):
                    self.global_config["MANUAL_CODING_ENABLED"] = value.lower() in (
                        "true",
                        "ja",
                        "yes",
                        "1",
                ...
                print(
                    f"🩹 MANUAL_CODING_ENABLED = {self.global_config['MANUAL_CODING_ENABLED']}"
                )
            else:
            ...
            if "ATTRIBUTE_LABELS" in self.global_config:
                print(
                    f"🩹 ATTRIBUTE_LABELS geladen: {self.global_config['ATTRIBUTE_LABELS']}"
                )
            else:
                ...
                print(
                    f"   Verwende Standard-ATTRIBUTE_LABELS: {self.global_config['ATTRIBUTE_LABELS']}"
                )
