...
class CategoryData:
    ...
    def validate(self) -> Tuple[bool, List[str]]:
        ...
        if not self.name or not self.name.strip():
            ...
            errors.append(
                f"Category name too short (minimum 3 characters), got {len(self.name)}"
            )
        elif len(self.name) > 50:
            errors.append(
                f"Category name too long (maximum 50 characters), got {len(self.name)}"
            )
...
class CodebookData:
    ...
    def validate(self, analysis_mode: Optional[str] = None) -> Tuple[bool, List[str]]:
        """
        Validiert Codebook.

        Args:
            analysis_mode: Optional Analyse-Modus ("grounded" / "deductive" / ...).
                Im "grounded" Mode sind keine deduktiven Kategorien erforderlich –
                es zählt nur die Forschungsfrage.
        ...
        # Validate research question (in ALL modes mandatory)
        ...
        if not isinstance(self.kodierregeln, dict):
            ...
            if missing_keys:
                errors.append(
                    f"Missing coding rule categories: {', '.join(missing_keys)}"
                )
        ...
        # GROUNDED MODE: Only the research question is mandatory.
        # Categories are generated inductively during analysis – no prior
        # definition required.
        if analysis_mode == "grounded":
            return len(errors) == 0, errors

        # Validate categories (alle anderen Modi)
        if not isinstance(self.deduktive_kategorien, dict):
            ...
            for cat_name, category in list(self.deduktive_kategorien.items()):
                ...
                if cat_type_name == "CategoryData":
                    ...
                    self.deduktive_kategorien[cat_name] = CategoryData.from_dict(
                        category
                    )
                    category = self.deduktive_kategorien[cat_name]
                else:
                    errors.append(
                        f"Category '{cat_name}' has unexpected type: {cat_type_name}"
                    )
                    continue
                ...
                if not is_valid:
                    errors.extend(
                        [f"Category '{cat_name}': {err}" for err in cat_errors]
                    )
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "deduktive_kategorien": {
                name: cat.to_dict() for name, cat in self.deduktive_kategorien.items()
            },
        }
        ...
        if "deduktive_kategorien" in data:
            for name, cat_data in data["deduktive_kategorien"].items():
                if isinstance(cat_data, CategoryData):
                    ...
                    print(
                        f"[WARN] Category '{name}' has unexpected type: {type(cat_data).__name__}"
                    )
