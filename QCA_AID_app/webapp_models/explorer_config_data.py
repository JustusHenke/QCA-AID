...
class AnalysisConfig:
    ...
    def validate(self) -> Tuple[bool, List[str]]:
        ...
        if not isinstance(self.analysis_type, str):
            errors.append(
                f"Analysis type must be string, got {type(self.analysis_type).__name__}"
            )
        ...
        if not isinstance(self.filters, dict):
            errors.append(
                f"Filters must be dictionary, got {type(self.filters).__name__}"
            )
        else:
            valid_filter_keys = {
                "Dokument",
        ...
        if not isinstance(self.params, dict):
            errors.append(
                f"Params must be dictionary, got {type(self.params).__name__}"
            )
        else:
    ...
    def _validate_network_params(self) -> List[str]:
        ...
        if "node_size_factor" in self.params:
            ...
            if not isinstance(val, (int, float)):
                errors.append(
                    f"node_size_factor must be numeric, got {type(val).__name__}"
                )
            elif val <= 0:
        ...
        if "layout_iterations" in self.params:
            ...
            if not isinstance(val, int):
                errors.append(
                    f"layout_iterations must be integer, got {type(val).__name__}"
                )
            elif val <= 0:
    ...
    def _validate_heatmap_params(self) -> List[str]:
        ...
        for param in required:
            if param not in self.params:
                ...
                errors.append(
                    f"{param} must be string, got {type(self.params[param]).__name__}"
                )

        if "annot" in self.params and not isinstance(self.params["annot"], bool):
            errors.append(
                f"annot must be boolean, got {type(self.params['annot']).__name__}"
            )
    ...
    def _validate_summary_params(self) -> List[str]:
        ...
        if "text_column" not in self.params:
            ...
            errors.append(
                f"text_column must be string, got {type(self.params['text_column']).__name__}"
            )
        ...
        if self.analysis_type == "custom_summary":
            if "prompt_template" not in self.params:
                ...
                errors.append(
                    f"prompt_template must be string, got {type(self.params['prompt_template']).__name__}"
                )
    ...
    @classmethod
    def create_default(
        cls, analysis_type: str, name: Optional[str] = None
    ) -> "AnalysisConfig":
        """
...
class ExplorerConfigData:
    ...
    enabled_charts: List[str] = field(
        default_factory=lambda: ["category_distribution", "confidence_histogram"]
    )
    ...
    def validate(self) -> Tuple[bool, List[str]]:
        ...
        if not isinstance(self.base_config, dict):
            errors.append(
                f"base_config must be dictionary, got {type(self.base_config).__name__}"
            )
        else:
        ...
        if not isinstance(self.analysis_configs, list):
            errors.append(
                f"analysis_configs must be list, got {type(self.analysis_configs).__name__}"
            )
        else:
            for i, config in enumerate(self.analysis_configs):
                if not isinstance(config, AnalysisConfig):
                    errors.append(
                        f"analysis_configs[{i}] must be AnalysisConfig instance"
                    )
                else:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ExplorerConfigData":
        ...
        return cls(base_config=base_config, analysis_configs=analysis_configs)
    ...
    def create_default(cls) -> "ExplorerConfigData":
        ...
        analysis_configs = [AnalysisConfig.create_default("netzwerk")]
