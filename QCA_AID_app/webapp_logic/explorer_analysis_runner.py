...
from webapp_models.explorer_config_data import AnalysisConfig, ExplorerConfigData
...
class ExplorerAnalysisRunner:
    ...
    async def initialize(self) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            base_url = self.config_data.base_config.get(
                "api_base_url", None
            )  # Custom Base URL (z.B. GWDG)
            api_key_env = self.config_data.base_config.get(
                "api_key_env", None
            )  # Custom Env-Var-Name
            messages.append(
                f"Initialisiere LLM Provider: {provider_name} mit Modell: {model_name}"
            )
            if base_url:
                messages.append(f"Custom Base URL: {base_url}")
            if api_key_env:
                messages.append(f"Custom API Key Env: {api_key_env}")
            ...
            if "script_dir" not in base_config or not base_config["script_dir"]:
                ...
                project_root = base_config.get("project_root") or os.environ.get(
                    "QCA_AID_PROJECT_ROOT"
                )
                if project_root:
                    base_config["script_dir"] = str(project_root)

            self.analyzer = QCAAnalyzer(self.excel_path, self.llm_provider, base_config)
    ...
    async def run_all_analyses(self) -> Tuple[bool, List[str]]:
        ...
        similarity_threshold = float(
            self.config_data.base_config.get("similarity_threshold", 0.7)
        )

        harmonization_performed = self.analyzer.perform_selective_harmonization(
            [
                self._convert_analysis_config_to_dict(ac)
                for ac in self.config_data.analysis_configs
            ],
            clean_keywords,
        ...
        for idx, analysis_config in enumerate(self.config_data.analysis_configs, 1):
            ...
            if not analysis_config.active:
                messages.append(
                    f"Überspringe deaktivierte Auswertung: {analysis_config.name}"
                )
                continue

            # Requirement 2.1: Execute active analyses
            messages.append(
                f"--- Auswertung {idx}/{len(self.config_data.analysis_configs)}: {analysis_config.name} ---"
            )

            try:
                success, analysis_messages = await self._execute_single_analysis(
                    analysis_config
                )
                ...
                if not success:
                    ...
                    messages.append(
                        f"✓ Analyse '{analysis_config.name}' erfolgreich abgeschlossen"
                    )
    ...
    async def _execute_single_analysis(
        ...
        if filtered_df.empty:
            messages.append(
                f"⚠️ WARNUNG: Keine Daten nach Filterung für Analyse '{analysis_config.name}'"
            )
        ...
        if self.analyzer.needs_keyword_harmonization(
            analysis_config.analysis_type, analysis_config.params
        ):
        ...
        output_prefix = (
            f"{analysis_config.name}_{filter_str}"
            if filter_str
            else analysis_config.name
        )
        ...
        try:
            if analysis_config.analysis_type == "netzwerk":
                ...
                prompt_template = analysis_config.params.get(
                    "prompt_template", self.default_prompts.get("paraphrase", None)
                )
                ...
                prompt_template = analysis_config.params.get(
                    "prompt_template", self.default_prompts.get("reasoning", None)
                )
                ...
                if not prompt_template:
                    messages.append(
                        "⚠️ Kein Prompt-Template für benutzerdefinierte Zusammenfassung angegeben"
                    )
                    return False, messages
                ...
                await self.analyzer.create_sentiment_analysis(
                    filtered_df, f"Sentiment_{output_prefix}", analysis_config.params
                )

            else:
                messages.append(
                    f"⚠️ Unbekannter Analysetyp '{analysis_config.analysis_type}'"
                )
                return False, messages
    ...
    def _convert_analysis_config_to_dict(
        self, analysis_config: AnalysisConfig
    ) -> Dict[str, Any]:
        """
