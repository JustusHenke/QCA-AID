...
from QCA_AID_app.webapp_models.codebook_data import CategoryData, CodebookData
...
class CodebookManager:
    ...
    def load_codebook(
        ...
        try:
            ...
            if format == "json":
                success, codebook_dict, load_errors = self._load_from_json(
                    str(file_path)
                )
            elif format == "xlsx":
                success, codebook_dict, load_errors = self._load_from_xlsx(
                    str(file_path)
                )
            else:
            ...
            try:
                ...
                return (
                    False,
                    None,
                    [f"Fehler beim Konvertieren des Codebooks: {str(e)}"],
                )
    ...
    def save_codebook(
        ...
        Args:
            ...
            analysis_mode: Optional Analyse-Modus (z.B. "grounded"). Wird an die
                Codebook-Validierung weitergereicht, damit im Grounded Mode keine
                (Sub-)Kategorien erforderlich sind.
        ...
        try:
            ...
            for cat_name, category in list(codebook.deduktive_kategorien.items()):
                ...
                if cat_type_name == "CategoryData":
                    ...
                    return False, [
                        f"Category '{cat_name}' has unexpected type: {cat_type_name}. Expected CategoryData or dict."
                    ]

            # Validiere Codebook vor dem Speichern (mode-aware)
            is_valid, validation_errors = codebook.validate(analysis_mode=analysis_mode)
            if not is_valid:
    ...
    def _save_to_json(
        self, codebook_dict: Dict, json_path: str
    ) -> Tuple[bool, List[str]]:
        """
    ...
    def _save_to_xlsx(
        self, codebook_dict: Dict, xlsx_path: str
    ) -> Tuple[bool, List[str]]:
        """
