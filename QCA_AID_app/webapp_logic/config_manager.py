...
class ConfigManager:
    ...
    def _load_config_cached(
        _self, file_path: str, file_hash: str, format: str
    ...
    def load_config(
        self, file_path: Optional[str] = None, format: Optional[str] = None
        ...
        try:
                ...
                if format is None:
                    if file_path.suffix.lower() in [".xlsx", ".xls"]:
                        ...
                        return (
                            False,
                            None,
                            [f"Unbekanntes Dateiformat: {file_path.suffix}"],
                        )
            else:
            ...
            success, config_dict, load_errors = self._load_config_cached(
                str(file_path), file_hash, format
            )
            ...
            try:
                ...
                return (
                    False,
                    None,
                    [f"Fehler beim Konvertieren der Konfiguration: {str(e)}"],
                )
    ...
    def save_config(
        self, config: ConfigData, file_path: Optional[str] = None, format: str = "json"
    ) -> Tuple[bool, List[str]]:
        """
    ...
    def get_default_config(self) -> ConfigData:
        ...
        config_dict = {
            ...
            "multiple_coding_threshold": DEFAULT_CONFIG.get(
            ...
            "attribute_labels": DEFAULT_CONFIG.get(
                "ATTRIBUTE_LABELS",
                {
                    "attribut1": "Attribut1",
                    "attribut2": "Attribut2",
            ...
            "coder_settings": DEFAULT_CONFIG.get(
                "CODER_SETTINGS",
                [
            ...
            "pdf_annotation_fuzzy_threshold": DEFAULT_CONFIG.get(
    ...
    def _load_from_xlsx(self, xlsx_path: str) -> Tuple[bool, Optional[Dict], List[str]]:
        ...
        try:
            ...
            config_dict = {
                "model_provider": global_config.get(
                ...
                "data_dir": global_config.get(
                    "data_dir", global_config.get("DATA_DIR", "input")
                ),
                "output_dir": global_config.get(
                ...
                "batch_size": int(
                    global_config.get("batch_size", global_config.get("BATCH_SIZE", 8))
                ),
                "code_with_context": self._convert_to_bool(
                    global_config.get(
                        "code_with_context",
                        global_config.get("CODE_WITH_CONTEXT", False),
                    )
                ),
                "multiple_codings": self._convert_to_bool(
                    global_config.get(
                ...
                "multiple_coding_threshold": float(
                    global_config.get(
                        ...
                        global_config.get("MULTIPLE_CODING_THRESHOLD", 0.85),
                    )
                ),
                "analysis_mode": global_config.get(
                    "analysis_mode", global_config.get("ANALYSIS_MODE", "deductive")
                ),
                "review_mode": global_config.get(
                    "review_mode", global_config.get("REVIEW_MODE", "consensus")
                ),
                "attribute_labels": global_config.get(
                    "attribute_labels", global_config.get("ATTRIBUTE_LABELS", {})
                ),
                "coder_settings": global_config.get(
                    "coder_settings", global_config.get("CODER_SETTINGS", [])
                ),
                "export_annotated_pdfs": self._convert_to_bool(
                    global_config.get(
                        "export_annotated_pdfs",
                        global_config.get("EXPORT_ANNOTATED_PDFS", True),
                    )
                ),
                "pdf_annotation_fuzzy_threshold": float(
                    global_config.get(
                        "pdf_annotation_fuzzy_threshold",
                        global_config.get("PDF_ANNOTATION_FUZZY_THRESHOLD", 0.85),
                    )
                ),
                "api_base_url": global_config.get(
                    "api_base_url", global_config.get("API_BASE_URL", None)
                ),
            ...
            print(
                f"  attribute_labels (Kleinbuchstaben): {global_config.get('attribute_labels', 'NICHT GEFUNDEN')}"
            )
            print(
                f"  ATTRIBUTE_LABELS (Großbuchstaben): {global_config.get('ATTRIBUTE_LABELS', 'NICHT GEFUNDEN')}"
            )
            ...
            print(
                f"  coder_settings (Kleinbuchstaben): {global_config.get('coder_settings', 'NICHT GEFUNDEN')}"
            )
            print(
                f"  CODER_SETTINGS (Großbuchstaben): {global_config.get('CODER_SETTINGS', 'NICHT GEFUNDEN')}"
            )
    ...
    def _save_to_json(
        self, config_dict: Dict, json_path: str
    ) -> Tuple[bool, List[str]]:
        """
        ...
        try:
            ...
            if Path(json_path).exists():
                try:
                    ...
                    json_data["forschungsfrage"] = existing_data.get(
                        "forschungsfrage", ""
                    )
                    json_data["kodierregeln"] = existing_data.get(
                        "kodierregeln", json_data["kodierregeln"]
                    )
                    json_data["deduktive_kategorien"] = existing_data.get(
    ...
    def _save_to_xlsx(
        self, config_dict: Dict, xlsx_path: str
    ) -> Tuple[bool, List[str]]:
        """
