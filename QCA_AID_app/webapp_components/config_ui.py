...
from webapp_models.config_data import CoderSetting, ConfigData
...
def render_project_root_section():
    ...
    with col2:
        ...
        if st.button(
            "📁 Ändern", use_container_width=True, help="Projekt-Verzeichnis ändern"
        ):
            # Open directory browser dialog
            new_root = FileBrowserService.open_directory_dialog(
                title="Projekt-Verzeichnis auswählen", initial_dir=current_root
            )
        ...
        with st.expander("📂 Konfiguration laden", expanded=True):
            ...
            with col_path:
                ...
                if file_path and file_path.strip():
                    ...
                    show_real_time_path_validation(
                        file_path, path_resolver, check_writable=False
                    )
                ...
                if st.button("Laden", key="load_config_btn", use_container_width=True):
                    ...
                    if selected_path:
                        ...
                        if not path_obj.is_absolute():
                            ...
                            path_obj = (
                                Path(project_manager.get_root_directory())
                                / selected_path
                            )
                        ...
                        success, config_data, errors = config_manager.load_config(
                            file_path=actual_path, format=load_format
                        )
                    else:
                    ...
                    if success:
                        ...
                        codebook_mgr = CodebookManager(
                            project_manager.get_root_directory()
                        )
                        cb_success, codebook_data, cb_errors = (
                            codebook_mgr.load_codebook(
                                file_path=actual_path, format=load_format
                            )
                        )
                        ...
                        cb_count = (
                            len(codebook_data.deduktive_kategorien)
                            if codebook_data
                            else 0
                        )
                        additional_info = {
                        ...
                        show_file_operation_success(
                            "geladen",
                            Path(actual_path) if actual_path else Path("(Standard)"),
                            additional_info,
                        )
                        ...
                        show_file_operation_error(
                            ...
                            Path(actual_path)
                            if actual_path
                            else Path(selected_path or "(Standard)"),
                            "\n".join(errors),
                            [
            ...
            with col_load2:
                if st.button(
                    "Abbrechen", key="cancel_load_config", use_container_width=True
                ):
                    # Clear selected path
                    st.session_state.selected_config_load_path = ""
                    st.session_state.show_load_dialog = False
        ...
        with st.expander("💾 Konfiguration speichern", expanded=True):
            ...
            save_format = st.radio(
                ...
                format_func=lambda x: (
                    "JSON (empfohlen)" if x == "json" else "Excel (XLSX)"
                ),
                horizontal=True,
            ...
            with col_path:
                ...
                current_config_filename = st.session_state.get(
                    "current_config_filename", f"QCA-AID-Codebook.{save_format}"
                )
                ...
                if (
                    current_path
                    and save_format != st.session_state.previous_save_format
                ):
                ...
                input_value = (
                    st.session_state.selected_config_save_path
                    if st.session_state.selected_config_save_path
                ...
                if file_path and file_path.strip():
                    ...
                    if not file_path_obj.is_absolute():
                        ...
                        file_path_obj = (
                            Path(project_manager.get_root_directory()) / file_path
                        )
                    show_real_time_path_validation(
                        str(file_path_obj), path_resolver, check_writable=True
                    )
            ...
            with col_save1:
                if st.button(
                    "Speichern", key="save_config_btn", use_container_width=True
                ):
                    config_manager = st.session_state.config_manager
                    config = st.session_state.config_data
                    ...
                    if selected_path:
                        ...
                        success, errors = config_manager.save_config(
                            config=config, file_path=actual_path, format=save_format
                        )
                    else:
                        # Use default filename
                        actual_path = str(
                            Path(project_manager.get_root_directory())
                            / f"QCA-AID-Codebook.{save_format}"
                        )
                        success, errors = config_manager.save_config(
                            config=config, format=save_format
                        )
                    ...
                    if success:
                        ...
                        st.session_state.current_config_filename = Path(
                            actual_path
                        ).name
                        ...
                        reload_success, reloaded_config, reload_errors = (
                            config_manager.load_config(
                                file_path=actual_path, format=save_format
                            )
                        )
                        ...
                        if reload_success and reloaded_config:
                            ...
                            st.warning(
                                "⚠️ Konfiguration gespeichert, aber Reload fehlgeschlagen. Bitte laden Sie die Datei manuell neu."
                            )
                        ...
                        show_file_operation_success(
                            "gespeichert", saved_path, additional_info
                        )
            ...
            with col_save2:
                if st.button(
                    "Abbrechen", key="cancel_save_config", use_container_width=True
                ):
                    # Clear selected path
                    st.session_state.selected_config_save_path = ""
                    st.session_state.show_save_dialog = False
        ...
        for config_file in config_files:
            ...
            for model in models:
                ...
                if provider_display_name in ["Mistral", "OpenRouter"]:
                    ...
                    if (
                        stored_id_without_prefix in config_id_without_prefix
                        or config_id_without_prefix in stored_id_without_prefix
                    ):
                        input_cost = model.get("cost_per_1m_in")
                        output_cost = model.get("cost_per_1m_out")
...
def check_api_key_for_provider(provider_display_name: str, provider_display_map: dict):
    ...
    Berücksichtigt auch eine custom_api_key_env aus der Konfiguration,
    falls eine Custom API Base URL gesetzt ist.
    ...
    # ── Vorrangprüfung: Custom API Base URL mit benanntem Env-Var ──
    # Defensiver Zugriff (getattr) für Backward-Compatibility mit alten Session-State-Objekten
    api_base_url = getattr(config, "api_base_url", None)
    custom_key_env = getattr(config, "custom_api_key_env", None)

    if api_base_url and custom_key_env:
        env_var = config.custom_api_key_env
        api_key = os.getenv(env_var)
        if api_key:
            masked = f"{api_key[:4]}...{api_key[-4:]}" if len(api_key) > 8 else "****"
            st.success(f"✅ API-Key `{env_var}` gefunden: `{masked}`")
        else:
            st.warning(
                f"⚠️ API-Key `{env_var}` nicht gefunden – "
                f"bitte in `.env` oder als Systemvariable setzen"
            )
        return
    ...
    if api_key_var is None:
        ...
        if provider_id == "local":
            st.info(
                "ℹ️ Lokale Modelle benötigen keinen API-Key. Stellen Sie sicher, dass LM Studio (Port 1234) oder Ollama (Port 11434) läuft."
            )
        return
        ...
        with st.expander("📖 Anleitung: API-Key einrichten", expanded=True):
            ...
            **Linux/Mac:**
            ...
            if provider_id == "openai":
                st.markdown(
                    "- Besuchen Sie: [https://platform.openai.com/api-keys](https://platform.openai.com/api-keys)"
                )
            elif provider_id == "anthropic":
                st.markdown(
                    "- Besuchen Sie: [https://console.anthropic.com/settings/keys](https://console.anthropic.com/settings/keys)"
                )
            elif provider_id == "mistral":
                st.markdown(
                    "- Besuchen Sie: [https://console.mistral.ai/api-keys/](https://console.mistral.ai/api-keys/)"
                )
            elif provider_id == "openrouter":
                st.markdown(
                    "- Besuchen Sie: [https://openrouter.ai/keys](https://openrouter.ai/keys)"
                )
...
def render_model_settings():
    ...
    if st.session_state.get("llm_provider_manager") and st.session_state.get(
        "llm_models_loaded"
    ):
        ...
        try:
            ...
            provider_options = [
                provider_display_map.get(p, p.title()) for p in all_providers
            ]

            # Map current config provider to display name
            current_provider_display = provider_display_map.get(
                config.model_provider.lower(), config.model_provider
            )
            current_provider_idx = (
                provider_options.index(current_provider_display)
                if current_provider_display in provider_options
                else 0
            )
        ...
        current_provider_idx = (
            provider_options.index(config.model_provider)
            if config.model_provider in provider_options
            else 0
        )
    ...
    api_base_url_val = getattr(config, "api_base_url", None)
    custom_api_key_env_val = getattr(config, "custom_api_key_env", None)
    using_custom_api = bool(api_base_url_val and api_base_url_val.strip())

    # ─────────────────────────────────────────────────────────────
    # CUSTOM API BASE URL — für ALLEN Provider sichtbar
    # ─────────────────────────────────────────────────────────────
    with st.expander("🔧 Custom API Base URL (OpenAI-kompatibel)", expanded=False):
        st.markdown("""
        **Wofür?** Ermöglicht die Verwendung von OpenAI-kompatiblen Endpoints, z.B.:
        - GWDG Academic Cloud: `https://chat-ai.academiccloud.de/v1`
        - Azure OpenAI: `https://your-resource.openai.azure.com/`
        - Andere OpenAI-kompatible Services

        **Wenn ausgefüllt:** Der Modell-Name wird zu einem Freitext-Feld,
        in das Sie jedes beliebige Modell eingeben können.

        Lassen Sie das Feld leer, um die Standard-URL des Providers zu verwenden.
        """)

        current_base_url = api_base_url_val or ""
        new_base_url = st.text_input(
            "API Base URL",
            value=current_base_url,
            placeholder="z.B. https://chat-ai.academiccloud.de/v1",
            help="Optional: Custom Base URL für OpenAI-kompatible Endpoints",
        )

        # Validate URL format if provided
        has_custom_url = bool(new_base_url and new_base_url.strip())
        if has_custom_url:
            if not (
        ...
        # Update config
        new_base_url_val = (
            new_base_url if (new_base_url and new_base_url.strip()) else None
        )
        if new_base_url_val != api_base_url_val:
            config.api_base_url = new_base_url_val
            st.session_state.config_modified = True

        # ── Custom API Key Env Var Name (nur wenn Custom Base URL gesetzt) ──
        if has_custom_url:
            st.markdown("---")
            st.markdown("**🔑 Custom API-Key-Umgebungsvariable**")
            st.info("""
            Geben Sie den Namen der Umgebungsvariable an, in der Ihr API-Key für
            diesen Custom-Endpoint gespeichert ist.

            **Beispiele:** `GWDG_API_KEY`, `AZURE_OPENAI_KEY`, `CUSTOM_API_KEY`

            Lassen Sie das Feld leer, um `OPENAI_API_KEY` zu verwenden.
            """)

            current_env_name = custom_api_key_env_val or ""
            new_env_name = st.text_input(
                "Env-Var-Name für API-Key",
                value=current_env_name,
                placeholder="OPENAI_API_KEY (Standard)",
                help="Name der Umgebungsvariable, die Ihren API-Key für diesen Endpoint enthält",
            )

            new_env_name_val = (
                new_env_name if (new_env_name and new_env_name.strip()) else None
            )
            if new_env_name_val != custom_api_key_env_val:
                config.custom_api_key_env = new_env_name_val
                custom_api_key_env_val = (
                    new_env_name_val  # lokale Variable aktualisieren
                )
                st.session_state.config_modified = True

            # Prüfe ob der benannte API-Key vorhanden ist
            import os

            env_to_check = new_env_name_val or "OPENAI_API_KEY"
            api_key_value = os.getenv(env_to_check)

            if api_key_value:
                masked = (
                    f"{api_key_value[:4]}...{api_key_value[-4:]}"
                    if len(api_key_value) > 8
                    else "****"
                )
                st.success(f"✅ API-Key `{env_to_check}` gefunden: `{masked}`")
            else:
                st.warning(
                    f"⚠️ API-Key `{env_to_check}` nicht in Umgebungsvariablen gefunden. Bitte in `.env` oder als Systemvariable setzen."
                )

    # ─────────────────────────────────────────────────────────────
    # MODEL NAME — Dropdown oder Freitext je nach API-Konfiguration
    # ─────────────────────────────────────────────────────────────
    # ── Entscheidung: Freitext-Modell wenn API Base URL im Textfeld gesetzt ist ──
    # Wir verwenden has_custom_url (das Textfeld) statt api_base_url_val (gespeicherte Config),
    # damit der Wechsel sofort passiert wenn der User etwas eingibt.
    if has_custom_url:
        # ── Freitext-Eingabe bei Custom API ──
        st.info(
            "ℹ️ Da eine Custom API Base URL konfiguriert ist, können Sie hier ein beliebiges Modell eingeben, das der Endpoint unterstützt."
        )

        new_model = st.text_input(
            "Modell-Name (Custom API)",
            value=config.model_name,
            placeholder="z.B. openai-gpt-oss-120b, gpt-4o-mini, deepseek-chat",
            help="Geben Sie den genauen Modell-Namen ein, den Ihr Custom-Endpoint bereitstellt",
        )
        ...
        # Pricing-Info bei Custom API (keine festen Preise bekannt)
        st.caption(
            "💰 💡 Kosten variieren je nach Custom-Endpoint — bitte beim Anbieter erfragen"
        )

    elif new_provider == "Local (LM Studio/Ollama)":
        # ── Lokale Modelle mit Erkennungsbutton ──
        col1, col2 = st.columns([3, 1])
        with col2:
            if st.button(
                "🔄 Erkennen",
                help="Sucht nach laufenden LM Studio oder Ollama Servern",
                ...
                with st.spinner("Suche nach lokalen Modellen..."):
                    try:
                        ...
                        from QCA_AID_assets.utils.llm.local_detector import (
                            LocalDetector,
                        )
                        ...
                        if local_models:
                            ...
                            if model_options:
                                st.success(
                                    f"✅ {len(model_options)} Modell(e) gefunden!"
                                )
                                st.session_state.local_models = model_options
                                st.rerun()
                            else:
                                st.warning(
                                    "⚠️ Keine Chat-Modelle gefunden (nur Embedding-Modelle)"
                                )
                        else:
                            st.warning(
                                "⚠️ Keine lokalen Modelle gefunden. Stellen Sie sicher, dass LM Studio oder Ollama läuft."
                            )
                    except Exception as e:
                        st.error(f"❌ Fehler: {e}")
        ...
        if "local_models" in st.session_state and st.session_state.local_models:
            ...
            current_model_idx = (
                model_options.index(config.model_name)
                if config.model_name in model_options
                else 0
            )
        else:
        ...
        new_model = st.selectbox(
            ...
            help="Wählen Sie das lokale Modell für die Analyse",
        )
        ...
        # ── Provider-spezifisches Dropdown (Standardfall) ──
        if st.session_state.get("llm_provider_manager") and st.session_state.get(
            "llm_models_loaded"
        ):
            manager = st.session_state.llm_provider_manager
            try:
                ...
                model_options = (
                    [config.model_name] if config.model_name else ["gpt-4o-mini"]
                )
                current_model_idx = 0
        else:
        ...
        if pricing_info:
            ...
            st.caption(
                f"💰 Kosten: \\${input_cost:.2f} / 1M Input-Tokens · \\${output_cost:.2f} / 1M Output-Tokens"
            )
        elif "local" in new_provider.lower():
            st.caption("💰 Kostenlos (lokales Modell)")
    ...
    if pricing_info:
        ...
        "local" in new_provider.lower()
        or "ollama" in new_model.lower()
        or "lm-studio" in new_model.lower()
    ):
        st.caption("💰 Kostenlos (lokales Modell)")
    ...
    if new_data_dir:
        ...
        if not input_path.exists():
            st.warning(
                f"⚠️ Verzeichnis existiert nicht und wird bei Bedarf erstellt: `{input_path}`"
            )
    ...
    if new_output_dir:
        ...
        if not output_path.exists():
            st.warning(
                f"⚠️ Verzeichnis existiert nicht und wird bei Bedarf erstellt: `{output_path}`"
            )
...
def render_analysis_settings():
    ...
    current_mode_idx = (
        analysis_mode_options.index(config.analysis_mode)
        if config.analysis_mode in analysis_mode_options
        else 0
    )
    ...
    current_review_idx = (
        review_mode_options.index(config.review_mode)
        if config.review_mode in review_mode_options
        else 0
    )
    ...
    **Wie funktioniert die Relevanz-Bewertung:**
    ...
    if new_relevance_threshold < 0.3:
        st.warning(
            f"⚠️ Wert unter 0.3: Inkludiert Segmente die das LLM als nicht relevant eingestuft hat (Confidence {new_relevance_threshold:.1f}-0.3)"
        )
    elif new_relevance_threshold > 0.5:
        st.info(
            f"ℹ️ Wert über 0.5: Sehr strenge Filterung, nur hochrelevante Segmente (Confidence ≥{new_relevance_threshold:.1f})"
        )
    ...
    if new_relevance_threshold > 0.0:
        st.info(
            f"ℹ️ Nur Segmente mit Relevanz-Konfidenz ≥ {new_relevance_threshold:.2f} werden analysiert."
        )
    else:
        st.info(
            "ℹ️ Alle vom LLM als relevant identifizierten Segmente werden analysiert (empfohlen)."
        )
...
def render_coder_settings():
    ...
    if manual_enabled:
        st.info(
            "ℹ️ Bei aktivierter manueller Kodierung wird nach der automatischen Kodierung ein Fenster geöffnet, in dem Sie jedes Segment manuell kodieren können."
        )
        ...
        with st.container():
            ...
            with col2:
                ...
                if new_id:
                    other_ids = [
                        c.coder_id
                        for j, c in enumerate(config.coder_settings)
                        if j != i
                    ]
...
def render_attribute_labels():
    ...
    if config.attribute_labels:
        ...
        preview_text = " | ".join(
            [f"{k}: {v}" for k, v in config.attribute_labels.items()]
        )
